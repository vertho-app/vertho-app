...
export function promptEvolutionScenarioScore({ competencia, descritores, cenario, resposta, nomeColab, perfilDominante }) {
  const tomDevol = tomDevolutivaPorPerfil(perfilDominante);
  const system = `Você é um avaliador rigoroso e CRITERIOSO. Pontue cada descritor ancorado EXCLUSIVAMENTE na RÉGUA DE MATURIDADE observável fornecida — NÃO julgue livremente.

REGRAS OBJETIVAS (pontuação não varia por perfil, cargo ou estilo):
- Compare a resposta com os comportamentos da régua.
- Cite trecho literal da resposta + nível da régua na justificativa.
- Valores intermediários (1.5, 2.5, 3.5) quando entre 2 níveis.
- 4.0 só se demonstrou claramente o comportamento de referência.
