...
**Decisão: seis meses corridos.** Conta-se do encerramento da conversa ou da última atualização de treino inativo; tentativas técnicas recentes também completam sua janela antes do expurgo conjunto. Cálculo em UTC com adição de meses (inclusive fevereiro/ano bissexto), não 180 dias. Lease viva impede exclusão. O cron `retencao_pace`, agendado em `vercel.json`, varre por tenant em lotes limitados; falhas/backlog são observáveis e não retornam sucesso silencioso.

Antes de expurgar, gera JSON compactado no bucket **privado** `backups/pace-retencao`, baixa de volta e confere os bytes. O banco trava sessão/tentativas, revalida a elegibilidade e compara o hash do snapshot completo: qualquer mudança posterior ao backup preserva o registro. Backups de segurança não aparecem no histórico e seguem a janela operacional existente de sete dias, com rotação exclusiva dessa pasta. Após restauração de backup, executar a manutenção antes de reabrir acesso ao acervo.

Excluir colaborador/empresa também exclui seus treinos PACE e tentativas pelas FKs compostas `CASCADE`; a confirmação de cadastro informa esse impacto. Não se alteram as FKs de outros módulos nesta migration; se outra dependência bloquear a exclusão, o usuário recebe erro compreensível, não o texto cru do Postgres. A aplicação não apaga registros ao simplesmente vencer o prazo comercial. A interface avisa o mascaramento de telefone, e-mail e CPF; CNPJ de 14 dígitos não é confundido com telefone. Use dados fictícios nos treinos.
