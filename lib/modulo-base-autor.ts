...
- COERÊNCIA INTERNA (o módulo é auditado contra si mesmo, e a contradição interna é o defeito nº 1). Duas regras:
  (a) guarda_corpos.preservar descreve o que não pode ser PERDIDO na adaptação — NÃO é regra que cada frase isolada precise satisfazer. NUNCA use quantificador universal sobre os itens do conteúdo ("toda abertura reúne X, Y e Z", "sempre", "em todos os exemplos", "cada fala"). O repertório tem frases curtas que naturalmente não carregam a estrutura inteira, e uma frase curta não pode virar violação de um guarda-corpo que você mesmo escreveu. Escreva "não perder o vínculo entre fato observável e impacto ao abrir uma conversa difícil", não "toda abertura deve conter fato, impacto e escuta".
  (b) Um exemplo só pode AFIRMAR que segue uma estrutura se nomear TODOS os elementos dela. Se escrever "traz as quatro camadas" ou "usa a sequência fato-impacto-escuta", as quatro/os três têm de estar visíveis na fala citada. Na dúvida, descreva o que a fala faz em vez de rotulá-la com a estrutura.
  Nada em repertorio_linguagem pode contrariar cuidados_eticos, e nenhuma orientação de ação pode ignorar uma ressalva que você mesmo escreveu. Antes de fechar o JSON, releia guarda_corpos e confirme que nenhum item vira uma regra que o próprio conteúdo descumpre.
