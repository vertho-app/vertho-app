...
- [ ] `auth.users` existe para cada e-mail — `select count(*) from colaboradores c left join auth.users u on lower(u.email)=lower(c.email) where c.empresa_id=… and u.id is null`. O import cria **0 contas** (é um INSERT em `colaboradores` e nada mais), então logo depois dele este número é o tamanho da turma. Desde 15/09/2026 as duas portas de link se auto-provisionam (`/api/auth/magic-link` e o disparo em lote do admin), então a conta nasce no primeiro pedido e a pessoa não bate mais em "Falha ao gerar link" — **mas** enquanto ninguém pede, a conta não existe: se você vai CONFERIR ou MEDIR antes de convidar, provisione na mão.
