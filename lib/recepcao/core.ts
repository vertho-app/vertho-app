...
export function sugerirNivel(
  ...
  for (const c of concluidas) {
    ...
    if (
      i >= 0 &&
      c.nota !== null &&
      c.nota >= (c.escalaNota === '1-4' ? 3 : NOTA_PARA_SUBIR)
    )
...
export const fichaParaPrompt = (c: Cenario) =>
  JSON.stringify({ ...c.publico, nivel: undefined });
export function ordenarPorNivel<
  T extends { ficha: { nivel?: string | null; titulo: string } },
>(itens: T[]): T[] {
  const pos = (n?: string | null) => {
    const i = NIVEIS.indexOf(n as Nivel);
    return i < 0 ? NIVEIS.length : i;
  };
  return [...itens].sort(
    (a, b) =>
      pos(a.ficha.nivel) - pos(b.ficha.nivel) ||
      a.ficha.titulo.localeCompare(b.ficha.titulo, 'pt-BR'),
  );
}

const exigir = (c: unknown, m: string) => {
...
export function validarCenario(c: Cenario) {
  if (c.matriz) cenarioSchema.parse(c);
  exigir(
  ...
  exigir(
    Array.isArray(c.publico.procedimentos) && c.publico.procedimentos.length,
    'Procedimentos obrigatórios',
  );
  exigir(
    Array.isArray(c.rubrica) && c.rubrica.length >= 3 && c.rubrica.length <= 7,
    'Entre três e sete dimensões obrigatórias',
  );
  exigir(
    new Set(c.rubrica.map((d) => d.id)).size === c.rubrica.length,
    'Dimensão duplicada',
  );
  exigir(
    c.rubrica.every(
  ...
  exigir(
    c.rubrica.reduce((s, d) => s + d.peso, 0) === 100,
    'Pesos devem somar 100',
  );
  exigir(
    Number.isInteger(c.limiteRespostas) &&
      c.limiteRespostas > 0 &&
      c.limiteRespostas <= 20,
    'Limite inválido',
  );
  exigir(
    Array.isArray(c.ocorrenciasCriticas) && Array.isArray(c.desfechos),
    'Vocabulários obrigatórios',
  );
  return c;
}
...
export function abrirSessao(cenario: Cenario, variante?: number): Estado {
  ...
  exigir(
    Number.isInteger(escolhida) && escolhida >= 0 && escolhida < opcoes.length,
    'Variante inválida',
  );
  c.paciente = opcoes[escolhida];
  c.variantes = [];
  return {
    id: String(randomUUID()),
    cenario: c,
    variante: escolhida,
    status: RECEPCAO_SESSAO.EM_ANDAMENTO,
    motivoFim: null,
    respostas: 0,
    revisao: 0,
...
export function visaoPublica(s: Estado) {
  return clone({
    ...
    relatorio: relatorioAtendimentoPublico(s.relatorio),
  });
}
...
export function validarFala(fala: string, c: Cenario) {
  exigir(
    texto(fala) && fala.length <= 800,
    'Fala inválida; saída não será truncada',
  );
  ...
  exigir(
    !/PERSONAGEM RESERVADO|FICHA OPERACIONAL|(?:system|developer)\s*prompt|"(?:comportamento|rubricaVersao|ocorrenciasCriticas)"\s*:/i.test(
      fala,
    ),
    'Exposição de instruções na fala',
  );
  for (const reservado of [c.paciente.comportamento, c.paciente.limites]) {
    exigir(
      !reservado || reservado.length < 45 || !fala.includes(reservado),
      'Exposição de instruções na fala',
    );
  }
}
...
export async function responder(
  s: Estado,
  { requestId, mensagem }: { requestId: string; mensagem: string },
  gerarTexto: Gerar,
) {
  exigir(texto(requestId) && requestId.length <= 100, 'requestId obrigatório');
  exigir(
    texto(mensagem) && mensagem.trim().length <= 4000,
    'Mensagem deve ter entre 1 e 4000 caracteres',
  );
  const conteudo = mensagem.trim();
  const recibo = s.recibos.find((r) => r.requestId === requestId);
  if (recibo) {
    exigir(
      recibo.mensagem === conteudo,
      'requestId reutilizado com outro conteúdo',
    );
  ...
  const historico: Estado['historico'] = [
    ...s.historico,
    { id: `m${s.historico.length}`, role: 'user', content: conteudo },
  ];
  const saida = parse(
    await gerarTexto({
      etapa: 'paciente',
  ...
  n.historico = [
    ...historico,
    {
      id: `m${historico.length}`,
      role: 'assistant',
      content: saida.fala.trim(),
    },
  ];
  n.respostas += 1;
  n.revisao += 1;
...
function validarReferencias(
  ...
  for (const [i, r] of refs.entries()) {
    ...
    if (
      m &&
      texto(r.trecho) &&
      !normalizarCitacao(m.content).includes(normalizarCitacao(r.trecho))
    ) {
      ...
      const candidatas = s.historico.filter(
        (x) =>
          (!papel || x.role === papel) &&
          normalizarCitacao(x.content).includes(normalizarCitacao(r.trecho)),
      );
    ...
    if (
      ...
      throw new ErroReferenciaAvaliacao(
        'citacao_invalida',
        `${campo}[${i}]`,
        'Citação inexistente ou não literal. Copie um trecho exato da mensagem indicada.',
      );
    }
...
export function consolidar(s: Estado, insumos: Insumos): Estado['relatorio'] {
  const rubrica = rubricaAvaliavel(s.cenario);
  exigir(
    Array.isArray(insumos?.dimensoes) &&
      insumos.dimensoes.length === rubrica.length,
    'Dimensões incompletas',
  );
  exigir(
    new Set(insumos.dimensoes.map((d) => d.id)).size === rubrica.length,
    'Dimensão duplicada',
  );
  ...
  const valores: Record<string, number> =
    escala === 'n4'
      ? { n1: 0, n2: 1, n3: 2, n4: 3 }
      : { adequado: 2, parcial: 1, insuficiente: 0 };
  const teto = escala === 'n4' ? 3 : 2,
  ...
  const dimensoes = rubrica.map((r) => {
    ...
    exigir(
      d && texto(d.justificativa),
      'Dimensão ausente ou sem justificativa',
    );
    exigir(
      Object.hasOwn(valores, d.classificacao) ||
    ...
    if (d.classificacao === 'nao_observavel') {
      exigir(
        ...
        'Não observável não pode declarar evidência nem oportunidade',
      );
    } else {
      exigir(
        d.oportunidades.length > 0,
        'Dimensão avaliada exige oportunidade',
      );
      if (d.classificacao !== piso)
        exigir(d.evidencias.length > 0, 'Mérito exige evidência');
  ...
  for (const [i, o] of insumos.ocorrencias.entries()) {
    exigir(
      s.cenario.ocorrenciasCriticas.includes(o.categoria) && texto(o.motivo),
      'Ocorrência inválida',
    );
  ...
  exigir(
    s.cenario.desfechos.includes(desfecho?.tipo) &&
      texto(desfecho.justificativa),
    'Desfecho inválido',
  );
  ...
  if (desfechoExigeAcordo(desfecho.tipo)) {
    const papeis = new Set(
      desfecho.evidencias.map(
        (r) => s.historico.find((m) => m.id === r.mensagemId).role,
      ),
    );
    exigir(
      papeis.has('user') && papeis.has('assistant'),
      'Resolução exige combinado e aceitação',
    );
  }
  exigir(
    ['acerto', 'melhoria', 'novaTentativa'].every((k) =>
      texto(insumos.feedback?.[k]),
    ),
    'Feedback incompleto',
  );
  ...
  const feedback = {
    ...
    melhoria: limpar(insumos.feedback.melhoria),
    novaTentativa: limpar(insumos.feedback.novaTentativa),
  };
  ...
  return {
    ...
    versaoRubrica: s.cenario.rubricaVersao,
    nota: competencias
      ? notaMatriz
      : pesoObservado
        ? Math.round((1000 * pontos) / pesoObservado) / 10
        : null,
    ...(competencias
      ? {
          competencias,
    ...
    situacao: insumos.ocorrencias.length
      ? 'atencao_critica'
      : pesoObservado < 100
        ? 'avaliacao_parcial'
        : 'avaliado',
    dimensoes,
    ocorrencias: clone(insumos.ocorrencias).map((o) => ({
      ...o,
      motivo: limpar(o.motivo),
    })),
    desfecho: {
      ...clone(desfecho),
      justificativa: limpar(desfecho.justificativa),
    },
    feedback,
  };
}
...
export async function encerrar(
  ...
  for (let tentativa = 0; tentativa < 2; tentativa++) {
    ...
    try {
      ...
      return {
        ...clone(s),
        status: RECEPCAO_SESSAO.CONCLUIDA,
        motivoFim: s.motivoFim ?? 'encerramento_usuario',
        revisao: s.revisao + 1,
        relatorio,
      };
    } catch (e) {
