...
export function gerarLeituraExecutiva(resultado) {
  ...
  // Bloco mais forte e mais fraco — ignora blocos excluídos (score null)
  const blocosArr = Object.entries(blocos)
    .map(([k, v]) => ({ nome: k, score: v?.score }))
    .filter(b => b.score != null);
  blocosArr.sort((a, b) => b.score - a.score);
  const maisForte = blocosArr[0] || { nome: '—', score: 0 };
  const maisFraco = blocosArr[blocosArr.length - 1] || { nome: '—', score: 0 };

  const nomesBlocos = { mapeamento: 'mapeamento comportamental', competencias: 'competências', lideranca: 'liderança', disc: 'Mapeamento Comportamental' };
