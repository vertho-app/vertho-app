...
| **Mapeamento Comportamental (DISC natural)** | Instrumento focado no perfil natural: 8 rankings + 6 escolhas forçadas, preferências de aprendizagem, **vídeo de instruções** e relatório detalhado (16 competências naturais + insights executivos por IA). O antigo perfil adaptado foi retirado do mapeamento e dos relatórios | Autoconhecimento com uma leitura única e coerente, sem misturar comportamento espontâneo com contexto de trabalho. |
...
| **Microconteúdos multi-formato** | Por semana: vídeo (avatar apresentador HeyGen + cenas animadas Remotion + narração própria, entregue via Bunny), texto, podcast (áudio gerado por IA, masterizado com vinhetas de marca), case study, desafio | Aprende no formato que prefere — mesmo conteúdo, várias entradas. |
| **Vídeos de microlearning personalizados** | Vídeo de 3-5 min gerado a partir do Módulo-Base: avatar apresentador na abertura e no fecho, cenas animadas no miolo (texto, ícones, comparações, passos, dados, frases), narração com voz própria e legendas. Roteiro por IA (Claude Opus 4.8), avatar com lip-sync da narração e streaming adaptativo (240p a 1080p). Feito sob medida pela combinação **cargo × perfil comportamental (DISC dominante) × PPP da escola** da pessoa e reaproveitado por todos os colaboradores da mesma célula (mesmo cargo + perfil) | Cada colaborador recebe um vídeo que fala a língua do cargo dele, no tom do seu perfil e com a cara da escola — produção em escala, sem gravação. |
...
| **Tira-Dúvidas (chat reativo)** | Chat com Claude Sonnet 4.6, com grounding nos valores e cultura da empresa (RAG) | Resposta na hora, contextualizada à empresa do colaborador — não genérica. |
...
| **Engajamento da trilha (telemetria)** *(jul-set/2026)* | Workspace único em `/admin/engajamento`, com abas **Visão atual** e **Evolução semanal**: trilha visual de acesso → consumo → evidência, etapa individual separada do calendário da turma, trajetórias, recuperados, heatmap por área e fila operacional de risco. A rota longitudinal antiga redireciona para a aba correspondente | Distingue “recebeu” de “engajou” e “a turma chegou à semana N” de “esta pessoa concluiu até N−1” — RH identifica pendências reais, recuperação e áreas que precisam de apoio sem perder contexto ao trocar de visão. |
| **Índice operacional transparente** | Ativação 20 + consumo 30 + evidência 40 + Tira-Dúvidas 10; o denominador longitudinal considera quem chegou à semana pelo calendário da cadência, enquanto a etapa individual usa conclusão sequencial | Permite acompanhar sinal da coorte e pendência da pessoa com réguas distintas e explicáveis, sem vender o indicador como nota, competência ou desempenho individual. |
...
| **Painéis Admin Vertho (internos)** | Evidências, Avaliação Acumulada, Auditoria Sem 14, Módulos-Base, Custos de IA (FinOps), Precificação comercial | Time Vertho consegue auditar/regerar avaliações, acompanhar o custo real e projetado e precificar com margem. |
...
| **Arguição — defesa oral (2º instrumento)** | Depois das 4 perguntas escritas, a IA abre uma conversa por turnos que SONDA a resposta (critério, robustez sob variação, limite reconhecido) e modula a nota via regra de código (±0,5, auditável) — não é chat livre, é defesa da tese. Validada no piloto, ligada em todos os modos (fechamento da sem 14 no regular) | Triangulação de método: o que a resposta preparada esconde, a sustentação ao vivo revela — e o avaliado sente o rigor do instrumento, não só um formulário. |
...
| Feature | O que é | Benefício |
|---|---|---|
| **Núcleo T0/T2** | 12 perguntas Likert + 1 aberta em 6 dimensões (clareza, condições, liderança, segurança para aprender, aplicação prática, futuro e permanência); no v2, esse núcleo é acompanhado pelo bloco contextual abaixo | Linha de base + medida final consistentes, sem reinventar a roda a cada ciclo. |
...
| **Contexto comportamental no Pulso v2** | Além das 12 Likert e 1 aberta, cada momento novo inclui 8 rankings + 6 escolhas forçadas. O resultado fica versionado no assignment e não sobrescreve o DISC natural | Preserva as perguntas do antigo modo adaptado no lugar certo: contexto de trabalho ao longo da jornada, sem poluir o perfil comportamental principal. |
...
>
> **Novidades 20-28/07/2026:** **Modo Personalizado** (seção 5.5) — a degustação virou builder de 1-4 semanas / 1-2 competências / fechamento opcional, com a config congelada na trilha. **Certificado de Conclusão** com critério de participação ≥ 75% (piloto não emite). **Puxar cores do site do cliente** no branding. **Engajamento da trilha** ganhou a visão longitudinal — hoje consolidada como a aba **Evolução semanal** — com trajetórias, recuperados e heatmap por área. O **perfil adaptado saiu do mapeamento e dos relatórios**; suas perguntas de contexto foram preservadas no Pulso v2. **Lotes de IA em segundo plano** com Batch API (−50%) e botão de parar. Na frente de segurança: auditoria multi-agente de 223 arquivos remediada de ponta a ponta, e o isolamento multi-tenant descrito na seção 4 passou a ser **verificado por guards no CI**, não só por convenção de código.
