...
async function montarSemanaConteudo(semana, descritorSel, competencia, cargo, contexto, prioridadeFormatos, empresaId, aiConfig, idsJaUsados = new Set()) {
  ...
  const poolBase = matchDescritor.length > 0 ? matchDescritor : (candidatos || []);
  // Remove conteúdos já usados em outras semanas (evita duplicação quando
  // o mesmo descritor ocupa 2 slots)
  const poolDisp = poolBase.filter(c => !idsJaUsados.has(c.id));
  // Se sobrou nenhum, recai no pool original (melhor repetir do que vazio)
  const pool = poolDisp.length > 0 ? poolDisp : poolBase;
