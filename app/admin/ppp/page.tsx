  ...
  try {
    ...
    if (fields.length > 0) {
      ...
      for (const f of fields) {
        ...
        let value = '';
        try {
          // constructor.name pode ser minificado em prod — usar duck typing
  ...
  if (!empresaIdParam || !empresa) {
    return (
      <div className="max-w-[1100px] mx-auto px-4 py-6 text-center">
        ...
        <button onClick={() => router.push('/admin/dashboard')} className="mt-4 text-cyan-400 text-sm hover:underline">{t('backDashboard')}</button>
      </div>
    );
  }
    ...
    <div className="max-w-[1100px] mx-auto px-4 py-6 sm:px-6" style={{ minHeight: '100dvh' }}>
      ...
      <div className="flex items-center gap-2 mb-4 flex-wrap">
        <button onClick={novaExtracao}
          ...
          <Plus size={14} /> {t('actions.newExtraction')}
        </button>
        {showForm && (
        ...
        <a href="/api/ppp/template" download
          ...
          <FileText size={14} /> {t('actions.downloadPdfForm')}
        </a>
        ...
        <div className="rounded-xl border border-white/[0.06] p-5 mb-6" style={{ background: '#0F2A4A' }}>
          ...
          <div className="mb-5">
            ...
            <p className="text-[10px] text-gray-500 mt-1.5">
              {t.rich('schoolEmptyHint', { strong: (chunks) => <strong className="text-amber-300/80">{chunks}</strong> })}
            </p>
          </div>
            ...
            <>
                  ...
                  <div className="mt-2 space-y-1">
                    {files.map((f, i) => (
                      <div key={i} className="flex items-center gap-2 px-3 py-2 rounded-lg" style={{ background: '#091D35' }}>
                        ...
                        {f.error && <span className="text-[9px] text-red-400 font-semibold">{t('status.error')}</span>}
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">{t('fields.urls')}</p>
                  <button onClick={() => setUrls(prev => [...prev, ''])}
                    className="text-[10px] font-bold text-green-400 hover:text-green-300">{t('actions.addUrl')}</button>
                </div>
                ...
                <p className="text-[10px] text-gray-600 mt-1">{t('urlHint')}</p>
              </div>
            </>
          )}
          ...
          {tab === 'json' && (
            <div>
              <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-2">{t('fields.pasteContent')}</p>
              <textarea value={textos} onChange={e => setTextos(e.target.value)}
                rows={8} placeholder={t('placeholders.pasteContent')}
          ...
          <div className="mt-4 mb-3">
            <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-2">{t('fields.aiModel')}</p>
          ...
          <label className="flex items-center gap-2 mt-3 cursor-pointer select-none">
            ...
            <span className="text-xs text-gray-300">{t('fields.enrichWeb')}</span>
            <span className="text-[9px] text-gray-600">{t('fields.enrichWebHint')}</span>
          </label>
      ...
      <div className="rounded-xl border border-white/[0.06] overflow-hidden" style={{ background: '#0F2A4A' }}>
        <div className="px-5 py-3 border-b border-white/[0.06] flex items-center justify-between">
          <span className="flex items-center gap-2 text-sm font-bold text-white">
            <FileText size={16} className="text-cyan-400" /> {t('listTitle', { count: ppps.length })}
          </span>
              ...
              <div key={p.id} className="flex items-center gap-3 px-5 py-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-gray-500">
                    {p.fonte || t('fallback.source')} — {p.created_at ? new Date(p.created_at).toLocaleDateString(locale) : '—'}
                    {Array.isArray(p.valores) && p.valores.length ? ` — ${t('valuesCount', { count: p.valores.length })}` : ''}
                  </p>
                </div>
                ...
                }`}>{p.status === 'extraido' ? t('status.extracted') : p.status === 'erro' ? t('status.error') : t('status.pending')}</span>
      ...
      {viewPPP && (() => {
        ...
        const origemLabels = {
          documento_interno: t('viewer.origins.internalDocument'),
          site_institucional: t('viewer.origins.website'),
          release_noticia: t('viewer.origins.news'),
          nao_identificado: t('viewer.origins.notAvailable'),
        };
        const confidenceLabel = (value) => {
          if (value === 'alta' || value === 'media' || value === 'baixa') return t(`viewer.confidence.${value}`);
          return value;
        };
        const ConfBadge = ({ confianca, origem }: { confianca?: any; origem?: any }) => (confianca || origem) ? (
          <div className="flex items-center gap-1.5 mt-1">
            {confianca && <span className={`text-[8px] font-bold px-1.5 py-0.5 rounded ${confiancaColors[confianca] || 'bg-gray-400/15 text-gray-400'}`}>{confidenceLabel(confianca)}</span>}
            {origem && <span className="text-[8px] text-gray-600">{origemLabels[origem] || origem}</span>}
          </div>
        ) : null;
        const Section = ({ num, title, color, confianca, origem, children }: { num?: any; title?: any; color?: any; confianca?: any; origem?: any; children?: any }) => (
          <div className="mb-4">
            <div className="flex items-center gap-2 mb-2">
              <p className="text-[10px] font-bold uppercase tracking-widest" style={{ color }}>{num}. {title}</p>
              {confianca && <span className={`text-[8px] font-bold px-1.5 py-0.5 rounded ${confiancaColors[confianca] || ''}`}>{confidenceLabel(confianca)}</span>}
        ...
        const List = ({ items }) => items?.length ? <ul className="space-y-0.5">{items.map((it, i) => <li key={i} className="text-xs text-gray-400">• {typeof it === 'string' ? it : (it?.nome || it?.termo || (typeof it === 'object' ? JSON.stringify(it) : String(it)))}</li>)}</ul> : <p className="text-xs text-gray-500 italic">{t('viewer.notDeclared')}</p>;
            ...
            <div className="w-full max-w-[750px] rounded-2xl border border-white/[0.08] p-6 mb-10" style={{ background: '#0A1D35' }}>
              ...
              {ext ? (
                <div className="space-y-1">
                  ...
                  {ext.perfil_organizacional ? (
                    <>
                      ...
                      {(() => {
                        ...
                        return (
                          <Section num="1" title={t('viewer.sections.organizationalProfile')} color="#00B4D8" confianca={s1.confianca} origem={s1.origem}>
                            ...
                            ) : s1.c ? <p>{String(s1.c)}</p> : <p className="text-gray-500 italic">{t('viewer.notIdentified')}</p>}
                          </Section>
                        );
                      })()}
                      ...
                      {(() => {
                        ...
                        return (
                          <Section num="2" title={t('viewer.sections.marketStakeholders')} color="#22C55E" confianca={s2.confianca} origem={s2.origem}>
                            {s2.c && typeof s2.c === 'object' ? (
                              <>
                                {s2.c.clientes && <p><span className="text-gray-500 font-semibold">{t('viewer.labels.clients')}:</span> {String(s2.c.clientes)}</p>}
                                {s2.c.concorrencia && <p><span className="text-gray-500 font-semibold">{t('viewer.labels.competition')}:</span> {String(s2.c.concorrencia)}</p>}
                                <List items={s2.c.stakeholders_chave} />
                              </>
                            ) : s2.c ? <p>{String(s2.c)}</p> : <p className="text-gray-500 italic">{t('viewer.notIdentified')}</p>}
                          </Section>
                        );
                      })()}
                      ...
                      {(() => {
                        ...
                        return (
                          <Section num="3" title={t('viewer.sections.identityCulture')} color="#A78BFA" confianca={s3.confianca} origem={s3.origem}>
                            {s3.c && typeof s3.c === 'object' ? (
                              <>
                                {s3.c.missao && <p><span className="text-gray-500 font-semibold">{t('viewer.labels.mission')}:</span> {String(s3.c.missao)}</p>}
                                {s3.c.visao && <p><span className="text-gray-500 font-semibold">{t('viewer.labels.vision')}:</span> {String(s3.c.visao)}</p>}
                                <List items={Array.isArray(s3.c.valores) ? s3.c.valores : []} />
                                {s3.c.modelo_gestao && <p className="mt-1"><span className="text-gray-500 font-semibold">{t('viewer.labels.managementModel')}:</span> {String(s3.c.modelo_gestao)}</p>}
                                {s3.c.cultura_declarada && <p><span className="text-gray-500 font-semibold">{t('viewer.labels.culture')}:</span> {String(s3.c.cultura_declarada)}</p>}
                              </>
                            ) : s3.c ? <p>{String(s3.c)}</p> : <p className="text-gray-500 italic">{t('viewer.notIdentified')}</p>}
                          </Section>
                        );
                      })()}
                      ...
                      {(() => {
                        ...
                        return (
                          <Section num="4" title={t('viewer.sections.operationsProcesses')} color="#F59E0B" confianca={s4.confianca} origem={s4.origem}>
                            {items.length ? (
                              <div className="space-y-1">
                                {items.map((p, i) => (
                            ...
                            ) : <p className="text-gray-500 italic">{t('viewer.notIdentified')}</p>}
                          </Section>
                        );
                      })()}
                      ...
                      {(() => {
                        ...
                        return (
                          <Section num="5" title={t('viewer.sections.peopleModel')} color="#EC4899" confianca={s5.confianca} origem={s5.origem}>
                            {s5.c && typeof s5.c === 'object' ? (
                              <>
                                {s5.c.desenvolvimento && <p><span className="text-gray-500 font-semibold">{t('viewer.labels.development')}:</span> {String(s5.c.desenvolvimento)}</p>}
                                {s5.c.avaliacao && <p><span className="text-gray-500 font-semibold">{t('viewer.labels.evaluation')}:</span> {String(s5.c.avaliacao)}</p>}
                                {s5.c.carreira && <p><span className="text-gray-500 font-semibold">{t('viewer.labels.career')}:</span> {String(s5.c.carreira)}</p>}
                                {s5.c.diversidade_inclusao && <p><span className="text-gray-500 font-semibold">D&I:</span> {String(s5.c.diversidade_inclusao)}</p>}
                              </>
                            ) : s5.c ? <p>{String(s5.c)}</p> : <p className="text-gray-500 italic">{t('viewer.notIdentified')}</p>}
                          </Section>
                        );
                      })()}
                          ...
                          <Section num="6" title={t('viewer.sections.governanceDecision')} color="#06B6D4" confianca={s6.confianca} origem={s6.origem}>
                            {s6.c && typeof s6.c === 'object' ? (
                              <>
                                ...
                                {s6.c.tomada_decisao && <p><span className="text-gray-500 font-semibold">{t('viewer.labels.decisionMaking')}:</span> {String(s6.c.tomada_decisao)}</p>}
                                {s6.c.compliance && <p><span className="text-gray-500 font-semibold">Compliance:</span> {String(s6.c.compliance)}</p>}
                              </>
                            ) : s6.c ? <p>{String(s6.c)}</p> : <p className="text-gray-500 italic">{t('viewer.notIdentified')}</p>}
                          </Section>
                        );
                      })()}
                      ...
                      {(() => {
                        ...
                        return (
                          <Section num="7" title={t('viewer.sections.technologyResources')} color="#8B5CF6" confianca={s7.confianca} origem={s7.origem}>
                            {s7.c && typeof s7.c === 'object' ? (
                              <>
                                <p className="text-gray-500 font-semibold">{t('viewer.labels.tools')}:</p><List items={Array.isArray(s7.c.ferramentas) ? s7.c.ferramentas : []} />
                                <p className="text-gray-500 font-semibold mt-1">{t('viewer.labels.capabilities')}:</p><List items={Array.isArray(s7.c.capacidades) ? s7.c.capacidades : []} />
                                <p className="text-gray-500 font-semibold mt-1">{t('viewer.labels.limitations')}:</p><List items={Array.isArray(s7.c.limitacoes) ? s7.c.limitacoes : []} />
                              </>
                            ) : s7.c ? <p>{String(s7.c)}</p> : <p className="text-gray-500 italic">{t('viewer.notIdentified')}</p>}
                          </Section>
                        );
                      })()}
                      ...
                      {(() => {
                        ...
                        return (
                          <Section num="8" title={t('viewer.sections.challengesStrategy')} color="#EF4444" confianca={s8.confianca} origem={s8.origem}>
                            {s8.c && typeof s8.c === 'object' ? (
                              <>
                                <p className="text-gray-500 font-semibold">{t('viewer.labels.challenges')}:</p><List items={Array.isArray(s8.c.desafios) ? s8.c.desafios : []} />
                                <p className="text-gray-500 font-semibold mt-1">{t('viewer.labels.goals')}:</p><List items={Array.isArray(s8.c.metas) ? s8.c.metas : []} />
                                {s8.c.transformacoes && <p className="mt-1"><span className="text-gray-500 font-semibold">{t('viewer.labels.transformations')}:</span> {String(s8.c.transformacoes)}</p>}
                              </>
                            ) : s8.c ? <p>{String(s8.c)}</p> : <p className="text-gray-500 italic">{t('viewer.notIdentified')}</p>}
                          </Section>
                        );
                      })()}
                      ...
                      {(() => {
                        ...
                        return (
                          <Section num="9" title={t('viewer.sections.corporateVocabulary')} color="#F97316" confianca={s9.confianca} origem={s9.origem}>
                            {items?.length ? (
                              <div className="space-y-1">
                                {items.map((v, i) => (
                    ...
                    <>
                      ...
                      <Section num="1" title={t('viewer.sections.institutionProfile')} color="#00B4D8">
                        ...
                        ) : <p className="text-gray-500 italic">{t('viewer.notDeclared')}</p>}
                      </Section>

                      <Section num="2" title={t('viewer.sections.communityContext')} color="#22C55E">
                        <p>{ext.comunidade_contexto || t('viewer.notDeclared')}</p>
                      </Section>

                      <Section num="3" title={t('viewer.sections.identity')} color="#A78BFA">
                        {ext.identidade ? (
                          <>
                            <p><span className="text-gray-500 font-semibold">{t('viewer.labels.mission')}:</span> {ext.identidade.missao}</p>
                            <p><span className="text-gray-500 font-semibold">{t('viewer.labels.vision')}:</span> {ext.identidade.visao}</p>
                            <p className="mt-1"><span className="text-gray-500 font-semibold">{t('viewer.labels.principles')}:</span></p>
                      ...
                      <Section num="4" title={t('viewer.sections.describedPractices')} color="#F59E0B">
                      ...
                      <Section num="5" title={t('viewer.sections.inclusionDiversity')} color="#EC4899">
                      ...
                      <Section num="7" title={t('viewer.sections.infrastructureResources')} color="#8B5CF6">
                        {ext.infraestrutura_recursos ? (
                          <>
                            <p className="text-gray-500 font-semibold">{t('viewer.labels.spaces')}:</p><List items={ext.infraestrutura_recursos.espacos} />
                            <p className="text-gray-500 font-semibold mt-1">{t('viewer.labels.technology')}:</p><List items={ext.infraestrutura_recursos.tecnologia} />
                            <p className="text-gray-500 font-semibold mt-1">{t('viewer.labels.limitations')}:</p><List items={ext.infraestrutura_recursos.limitacoes} />
                          </>
                        ) : <p className="text-gray-500 italic">{t('viewer.notDeclared')}</p>}
                      </Section>

                      <Section num="8" title={t('viewer.sections.challengesGoals')} color="#EF4444">
                        {ext.desafios_metas ? (
                          <>
                            <p className="text-gray-500 font-semibold">{t('viewer.labels.challenges')}:</p><List items={ext.desafios_metas.desafios} />
                            <p className="text-gray-500 font-semibold mt-1">{t('viewer.labels.goals')}:</p><List items={ext.desafios_metas.metas} />
                          </>
                        ) : <p className="text-gray-500 italic">{t('viewer.notDeclared')}</p>}
                      </Section>

                      <Section num="9" title={t('viewer.sections.vocabulary')} color="#F97316">
                  ...
                  <Section num="10" title={t('viewer.sections.prioritizedCompetencies')} color="#10B981">
                  ...
                  {(() => {
                    ...
                    return Array.isArray(vals) && vals.length > 0 ? (
                      <div className="pt-3 border-t border-white/[0.06]">
                        <p className="text-[10px] font-bold text-cyan-400 uppercase tracking-widest mb-2">{t('viewer.labels.values')}</p>
                  ...
                  {ext._metadata && (
                    <div className="pt-3 mt-2 border-t border-white/[0.06] space-y-3">
                      {ext._metadata.lacunas?.length > 0 && (
                        <div>
                          <p className="text-[10px] font-bold text-amber-400 uppercase tracking-widest mb-1">{t('viewer.metadata.gaps')}</p>
                      ...
                      {ext._metadata.hipoteses_controladas?.length > 0 && (
                        <div>
                          <p className="text-[10px] font-bold text-purple-400 uppercase tracking-widest mb-1">{t('viewer.metadata.controlledHypotheses')}</p>
                      ...
                      {ext._metadata.recomendacao_validacao?.length > 0 && (
                        <div>
                          <p className="text-[10px] font-bold text-red-400 uppercase tracking-widest mb-1">{t('viewer.metadata.validateWithHr')}</p>
                      ...
                      {ext._metadata.secoes_enriquecidas_web?.length > 0 && (
                        <div>
                          <p className="text-[10px] font-bold text-blue-400 uppercase tracking-widest mb-1">{t('viewer.metadata.enrichedWeb')}</p>
                ...
                <p className="text-sm text-gray-500 text-center py-4">{t('viewer.noExtractionData')}</p>
              )}

              <div className="flex items-center gap-4 text-[10px] text-gray-600 pt-3 mt-3 border-t border-white/[0.04]">
                <span>{t('viewer.labels.source')}: {viewPPP.fonte}</span>
                {viewPPP.url_site && <span className="truncate max-w-[300px]">URL: {viewPPP.url_site}</span>}
                <span>{t('viewer.labels.date')}: {viewPPP.created_at ? new Date(viewPPP.created_at).toLocaleDateString(locale) : '—'}</span>
              </div>
