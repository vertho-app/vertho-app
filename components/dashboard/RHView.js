  ...
  useEffect(() => {
    ...
    async function fetchData() {
      ...
      try {
        ...
        const notaMedia = avaliacoesConcluidas > 0
          ? sessoes.reduce((sum, s) => sum + (s.nota_decimal || 0), 0) / avaliacoesConcluidas
          : 0;
        ...
        if (sessoes?.length) {
          ...
          setCompetencias(
            Object.entries(grouped)
              .map(([nome, { total, count }]) => ({ nome, media: Math.round((total / count) * 10) / 10, count }))
