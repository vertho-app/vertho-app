...
const TAREFAS = {
  ...
  cliente: 'sim_vendas_cliente',
  moderador: 'sim_vendas_moderador',
  ...
  return async <K extends Etapa>(
    ...
    for (let retry = 0; retry < 2; retry++) {
      ...
      if (remaining < 15000)
        throw new SimuladorError(
          503,
          'A resposta demorou mais que o previsto. Tente novamente para continuar deste ponto.',
        );
      const id = randomUUID(),
      ...
      try {
        ...
        const saved = await c.tdb
          .from('sim_vendas_tentativas')
          .update({
            status: VENDAS_TENTATIVA.ACEITA,
            resultado: result,
        ...
        if (saved.error)
          throw new SimuladorError(
            503,
            'A resposta foi gerada, mas não foi possível confirmar o salvamento. Atualize antes de reenviar.',
          );
        return result;
      } catch (e) {
        const finish = await c.tdb
          .from('sim_vendas_tentativas')
          .update({
