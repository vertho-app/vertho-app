/**
 * Captura do BOAS-VINDAS GENÉRICO — o vídeo que serve qualquer empresa que entrar:
 * entrar no app → mapeamento comportamental. PARA no mapeamento, de propósito (o
 * vídeo existe para levar ao primeiro acesso, não para explicar o programa inteiro).
 *
 * Clonado do `capture-macae.mts` em 10/09/2026, trocando o estúdio: aquele grava no
 * tenant `macae` e narra "projeto Educação Integral"; este grava no `acme-demo` e não
 * cita cliente nenhum. Roteiro em `storyboard.ts` → `BOASVINDAS_GERAL`.
 *
 * Estúdio: `acme-demo` com uma persona FICTÍCIA que este script CRIA no início e APAGA
 * no fim (inclusive em falha). Nenhuma pessoa real entra em tela — é isso que sustenta
 * a exceção de vídeo PÚBLICO em `lib/videos-publicos.ts`, já que ele é assistido antes
 * de existir login. Nada é salvo: o mapeamento não chega a "Ver meu perfil".
 *
 * ⚠️ O CARGO da persona decide se há competências/cenário no tenant. `Coordenador de
 * Operações` existe no acme-demo; um cargo que não exista deixa telas vazias e a
 * captura morre esperando elemento que nunca aparece (foi o que travou o `boasvindas`
 * da UniAnchieta). Conferir antes de trocar de estúdio:
 *   select cargo, count(*) from banco_cenarios where empresa_id = <estudio> group by cargo;
 *
 * Rodar (dev no ar):
 *   npx tsx video-spike/tutorial/capture-boasvindas-geral.mts
 */
...
const IMGDIR = path.join(PUBLIC_DIR, 'tutorial', 'boasvindas-geral');

const EMPRESA_ID = '455f9366-fb4f-4c58-a79e-f94193464744'; // ACME Demo — estúdio, sem PII
const PERSONA = {
  email: 'gravacao.boasvindas@vertho.ai', // @vertho.ai → fora de toda estatística
  nome_completo: 'Alex Demonstração',
  cargo: 'Coordenador de Operações', // existe no acme-demo, com competências
  role: 'colaborador',
};
...
async function shot(page: Page, id: string, bbox: Box | null = null) {
  ...
  const rel = `tutorial/boasvindas-geral/${id}.png`;
...
async function main() {
  ...
  try {
    ...
    // Espera a home saudar a persona. O primeiro nome sai do PERSONA, não de um
    // literal: no clone do flow de Macaé este `waitFor` procurava "Olá, Ana" e a
    // persona daqui é outra — trocar o nome quebrava a captura em silêncio.
    const primeiroNome = PERSONA.nome_completo.split(' ')[0];
    await page.getByText(new RegExp(`Olá,\\s*${primeiroNome}`, 'i')).waitFor({ timeout: 30000 });
    ...
    const out = path.join(OUT_DIR, 'boasvindas-geral.frames.json');
    writeFileSync(out, JSON.stringify({ flow: 'boasvindas-geral', viewport: { width: 1920, height: 1080 }, frames }, null, 2));
