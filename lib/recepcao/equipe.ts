...
export async function todas(query: () => any): Promise<any[]> {
  ...
  for (let n = 0; n < 10000; n += 500) {
    ...
    if (error)
      throw new RecepcaoError(
        503,
        'Não foi possível consultar os dados do treinamento.',
      );
    rows.push(...(data || []));
  ...
  throw new RecepcaoError(
    422,
    'Há muitos registros neste período. Use um período menor.',
  );
}
export async function pessoasDaEquipe(c: ContextoRecepcao) {
  ...
  const pessoas = await todas(() =>
    c.sb
      .from('colaboradores')
      .select('id,empresa_id,nome_completo,email,gestor_email')
...
export async function sessaoDaEquipe(c: ContextoRecepcao, id: string) {
  ...
  const { data, error } = await c.sb
    ...
    .maybeSingle();
  if (error)
    throw new RecepcaoError(503, 'Não foi possível consultar o atendimento.');
  if (
    !data ||
    (!c.auth.isPlatformAdmin &&
      !pessoas.some((p) => p.id === data.colaborador_id))
  )
...
export function resumirEquipe(rows: any[], pessoas: any[], revisoes: any[]) {
  ...
  return {
    ...
    pessoas: pessoas.map((p) => ({
      ...
      iniciadas: rows.filter((r) => r.colaborador_id === p.id).length,
      concluidas: concluidas.filter((r) => r.colaborador_id === p.id).length,
    })),
    grupos: [...grupos.values()].map((g) => ({
      ...g,
      media: g.notas.length
        ? g.notas.reduce((a, b) => a + b, 0) / g.notas.length
        : null,
      notas: undefined,
    })),
    sessoes: rows.map((r) => ({
      id: r.id,
      nome:
        pessoas.find((p) => p.id === r.colaborador_id)?.nome_completo ||
...
export async function painelEquipe(
  ...
  if (podeCustos) {
    ...
    const ledger: any[] = [];
    for (let i = 0; i < tentativas.length; i += 80)
      ledger.push(
        ...(await todas(() =>
          c.sb
            .from('ia_usage_log')
            .select('correlation_id,model,cost_usd,latency_ms,status')
            .eq('empresa_id', c.empresaId)
            .in(
              'correlation_id',
              tentativas.slice(i, i + 80).map((t) => t.id),
            )
            .order('id'),
        )),
      );
    ...
    for (const t of tentativas)
      if (t.estado === 'rejeitada')
        rejeicoes[t.erro_codigo || 'outro'] =
          (rejeicoes[t.erro_codigo || 'outro'] || 0) + 1;
    const porSessao = rows.map((r) => {
      const ts = tentativas.filter((t) => t.sessao_id === r.id);
      const usage = ledger.filter((l) =>
        ts.some((t) => t.id === l.correlation_id),
      );
      return {
        id: r.id,
        tentativas: ts.length,
        semCustoRegistrado: usage.filter((l) => l.cost_usd == null).length,
        custoConhecidoUsd: usage.reduce((n, l) => n + (l.cost_usd || 0), 0),
        semUsoRegistrado: ts.filter(
          (t) => !usage.some((l) => l.correlation_id === t.id),
        ).length,
      };
    });
    const avaliacoes = tentativas.filter((t) => t.etapa === 'avaliador'),
      recusadas = avaliacoes.filter((t) => t.estado === 'rejeitada');
    operacao = {
      tentativas: tentativas.length,
      rejeicoes,
      emAberto: tentativas.filter((t) => !t.finished_at).length,
      custoConhecidoUsd: ledger.reduce((n, l) => n + (l.cost_usd || 0), 0),
      modelos: [...new Set(ledger.map((l) => l.model))],
      porSessao,
      avaliacoes: avaliacoes.length,
      avaliacoesRejeitadas: recusadas.length,
      taxaRejeicao: avaliacoes.length
        ? (100 * recusadas.length) / avaliacoes.length
        : null,
    };
  }
