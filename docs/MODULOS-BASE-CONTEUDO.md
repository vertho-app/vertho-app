...
| `competencia_id` | uuid | sim | FK → catálogo Vertho (global) |
| `descritor_id` | uuid | sim | FK → régua de maturidade |
...
- `(descritor_id, nivel_entrada, nivel_destino)` **NÃO único** — múltiplos grupos podem coexistir (variantes pedagógicas).
...
- Cabeçalho conceitual (competência, descritor, níveis) deve continuar **igual entre variantes do mesmo grupo**.
...
## IA-as-autor (rascunho assistido)

Action `rascunharModuloBase({ competencia_id, descritor_id, nivel_entrada, nivel_destino, locale, contexto_pedagogico?, modulo_referencia_id? })`:

1. Carrega descrição da competência + descritor + nível da régua (entrada e destino).
2. Se `modulo_referencia_id` informado (caso de tradução), carrega esse módulo como referência.
3. Manda pro LLM com:
   - este spec como guia,
   - as descrições da régua,
   - amostra de módulos já publicados como exemplos (RAG sobre os módulos-base existentes).
4. Saída **validada contra o JSON schema** do corpo.
5. Persistido como **`status = rascunho`** atribuído ao autor logado.
6. Humano abre, edita o que quiser, manda pra revisão.

> O output da IA é **sempre rascunho** — nunca publica direto. A revisão humana é obrigatória.

---

## Como o engine consome (resolução)

Pra cada conteúdo a gerar `(colab, semana, competencia_foco, descritor, formato, locale)`:

1. **Resolve** módulos-base publicados em `(descritor_id, nivel_entrada=N atual do colab, nivel_destino=N+1)`.
