...
const cursorSchema = z.object({ em: z.string().datetime({ offset: true }), id: z.string().uuid() }).strict();
export function lerCursor(cursor?: string | null) {
  ...
  try {
    ...
    throw new SimuladorError(400, 'Página de histórico inválida. Atualize a lista.');
  }
}
export function aplicarCursor(query: any, cursor?: string | null) {
  ...
  return c ? query.or(`created_at.lt.${c.em},and(created_at.eq.${c.em},id.lt.${c.id})`) : query;
}
  ...
  return {
    ...
    proximoCursor:
      rows.length > tamanho && last
        ? Buffer.from(JSON.stringify({ em: last.created_at, id: last.id })).toString('base64url')
        : null,
  };
}
