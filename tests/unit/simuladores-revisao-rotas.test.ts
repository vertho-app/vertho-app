...
vi.mock('@/lib/auth/request-context', () => ({
  requireUser: vi.fn(async () => m.auth),
}));
vi.mock('@/lib/csrf', () => ({ csrfCheck: vi.fn(() => m.csrf) }));
vi.mock('@/lib/rate-limit', () => ({
  readLimiter: { check: vi.fn(async () => null) },
}));
vi.mock('@/lib/prontidao-lideranca/habilitado', () => ({
  prontidaoLiderancaHabilitada: vi.fn(async () => true),
}));
...
import {
  GET as getVendas,
  POST as postVendas,
} from '@/app/api/simulador-vendas/gestao/route';
...
const pedido = (url: string, body: unknown) =>
  new Request(`https://app.vertho.ai${url}`, {
    method: 'POST',
    body: typeof body === 'string' ? body : JSON.stringify(body),
  });
const pessoa = (role: string) => ({
  email: `${role}@cliente.test`,
  role,
...
describe('POST /api/simulador-vendas/gestao (revisão)', () => {
  it.each(['gestor', 'rh'])(
    '🔴 %s, que não treina, registra a revisão pelo contexto de leitura',
    async (role) => {
      m.auth = pessoa(role);
      const res = await postVendas(
        pedido('/api/simulador-vendas/gestao', corpo()),
      );
  ...
  it('sem CSRF válido ou com corpo inválido, nada chega à gravação', async () => {
    ...
    expect(
      (await postVendas(pedido('/api/simulador-vendas/gestao', corpo())))
        .status,
    ).toBe(403);
    m.csrf = null;
    for (const invalido of [
      corpo({ parecer: 'talvez' }),
      corpo({ motivo: '' }),
      'não é json',
    ])
      expect(
        (await postVendas(pedido('/api/simulador-vendas/gestao', invalido)))
          .status,
      ).toBe(400);
...
describe('POST /api/simulador-lideranca/equipe (revisão)', () => {
  it('RH registra a revisão da jornada', async () => {
    m.auth = pessoa('rh');
    const res = await postLideranca(
      pedido('/api/simulador-lideranca/equipe', corpo({ dimensoes: [] })),
    );
    expect(res.status).toBe(200);
  ...
  it('sem CSRF válido, com corpo inválido ou por quem não acompanha, nada chega à gravação', async () => {
    ...
    m.csrf = new Response('{}', { status: 403 });
    expect(
      (await postLideranca(pedido('/api/simulador-lideranca/equipe', corpo())))
        .status,
    ).toBe(403);
    m.csrf = null;
    expect(
      (
        await postLideranca(
          pedido('/api/simulador-lideranca/equipe', 'não é json'),
        )
      ).status,
    ).toBe(400);
    expect(
      (
        await postLideranca(
          pedido('/api/simulador-lideranca/equipe', corpo({ extra: 1 })),
        )
      ).status,
    ).toBe(400);
...
it('CSV mantém PL quando não há média por falta de cobertura', async () => {
  m.auth = { ...pessoa('rh'), isPlatformAdmin: true };
  sb = criarSupabaseMock({
    resolver: (t) =>
      t === 'empresas'
        ...
        : t === 'sim_vendas_config'
          ...
          : t === 'platform_admins'
            ? { id: 'admin' }
            : null,
    lista: (t) => (t === 'sim_vendas_sessoes' ? [{ id: 'sessao', PL: 3 }] : []),
  });
  ...
  const r = await getVendas(
    new Request(
      'https://app.vertho.ai/api/simulador-vendas/gestao?exportar=1&empresaId=' +
        EMP,
    ),
  );
  expect(r.status).toBe(200);
  expect((await r.json()).linhas[0]).toMatchObject({ PL: 3, Media: null });
});
