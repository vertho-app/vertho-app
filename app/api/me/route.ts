...
export async function GET() {
  try {
    ...
    // gestor veem quem faz e os resultados. A tela refaz a pergunta com as
    // permissões (`contextoEquipe`); aqui é só exibição do item de menu.
    const liderancaEquipe = liderancaEmpresa && (platformAdmin || ['rh', 'gestor'].includes(String((data as any)?.role ?? '')));
