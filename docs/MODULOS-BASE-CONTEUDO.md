...
3. `criarModulosDeTranscricao` → `segmentarTranscricao` fatia o texto em janelas (map-reduce), a IA-autora (`SYSTEM_AUTOR`) estrutura cada seção nos 4 blocos. **Direcionador** opcional (pilar/competência/**cargo**).
   - **MODO EXCLUSIVO** (quando há pilar ou competência): a extração é restrita ESTRITAMENTE ao escopo — o catálogo oferecido à IA contém SÓ as competências do pilar/competência escolhido (competência fixa a 1; só pilar fixa ao pilar inteiro), os fallbacks que "forçam" competência (re-tentativa sem direcionamento + fallback determinístico) são DESLIGADOS, e trechos fora do escopo são ignorados. Material não aderente → **0 módulos** + status **`vazio`** (mig 151, distinto de `error`/`done`; UI mostra "não aderente · 0 módulos").
   - **Erro de configuração é `error`, não `vazio`** (implementado em 16/09/2026; até lá a doc dizia isto e o código gravava `vazio`): pilar/competência inexistente, competência de nome ambíguo sem cargo, cargo sem competências e falha de leitura do catálogo voltam `erro` de `segmentarTranscricao` **antes de gastar IA**.
4. **Escopo empresa**: o segmentador usa o catálogo de competências DA EMPRESA, montado **por MATRIZ** (`montarCatalogoDaEmpresa`, `lib/matriz-por-cargo.ts`), não por nome. A matriz é gravada por cargo e o mesmo nome existe com descritores diferentes em cargos diferentes (Ibipeba: 8 nomes, entre eles "Autocuidado e resiliência emocional" em COO03 e DIR02):
   - cópias idênticas em vários cargos = **uma** entrada (o módulo-base é por matriz), ancorada na cópia do 1º cargo em ordem alfabética;
   - mesmo nome com descritores diferentes = entradas **separadas**, com os cargos no rótulo, cada uma só com os seus descritores;
   - o **cargo** do direcionamento (coluna `extracoes_video.cargo_direcionador`, mig 257; seletor na tela) filtra o catálogo às matrizes dele. Direcionar para um nome ambíguo **sem** cargo é recusado pedindo o cargo.
   O **descritor é escolhido da LISTA daquela matriz** (não texto livre — a IA copia um descritor real; `ancorarDescritor` é a rede de segurança, por id da entrada). Módulo aponta p/ `competencia_id`. Resolver por nome só vale para nome com uma matriz no escopo.
...
1. **Resolve a competência pelo NOME** nos DOIS catálogos: canônico (`competencias_base` → `competencia_base_id`) E o modelo da empresa (`competencias` → `competencia_id`, quando há `empresaId`).
   - **Com cargo, o módulo da empresa é POR MATRIZ, não por cargo** (decisão do dono, 16/09/2026). A matriz é gravada por cargo, então "a mesma matriz em 2 cargos" são duas cópias; o módulo ancorado numa cópia serve às cópias **idênticas** dos outros cargos (`idsDasCopiasEquivalentes`, `lib/matriz-por-cargo.ts`). "Idêntica" é pela **assinatura** (código + `cod_desc` + nome do descritor normalizado), nunca só pelo nome: em Ibipeba, "Autocuidado e resiliência emocional" existe em Coordenação (COO03) e Gestão Escolar (DIR02) com réguas diferentes, e cada uma continua vendo só o seu módulo. Cargo sem cópia da matriz não recebe módulo de outro cargo. Sem cargo ou `'todos'`: todos os ids do nome (como antes).
