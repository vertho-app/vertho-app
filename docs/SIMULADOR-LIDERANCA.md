...
**Revisão humana (19/09/2026, mig 264):** no detalhe da pessoa, quem acompanha registra se concorda com a avaliação da IA na jornada (concordo, concordo em parte, discordo), com motivo e as competências da matriz comentadas. Não altera nível nem média, não se edita, a própria pessoa não revisa a sua jornada, só depois da primeira devolutiva, e exige `assessments.answer` além da régua de acompanhamento. Tabela `sim_lideranca_revisoes` (só INSERT, `ON DELETE CASCADE` na jornada); rota `POST /api/simulador-lideranca/equipe`; administrador da plataforma revisa como `admin:<id>`. Núcleo e tela comuns ao vendas (`lib/simuladores/revisao.ts`, `components/simuladores/revisao-humana.tsx`). A pessoa não vê a revisão. Desde a migration 265, cada parecer fica vinculado ao recorte efetivamente revisado: referência das avaliações, matriz, encontros públicos e síntese. O servidor confere a referência antes de gravar; uma tela desatualizada recebe conflito, e o reenvio idêntico continua idempotente. Conversa integral, preparação e reflexão não entram nesse recorte. O painel resume a última revisão do recorte atual; quando a jornada avança, volta a ficar pendente. Revisões antigas sem recorte são identificadas como legadas, e falha de consulta aparece como indisponibilidade.
