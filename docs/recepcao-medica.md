...
- `RECEPCAO_NIVEIS_DB=1`: `node --env-file=.env.local node_modules/vitest/vitest.mjs run tests/unit/recepcao-niveis-db.test.ts` ensaia a publicação dos três degraus (1.1, 2.1, 3.1) em transação revertida, com backup; **`RECEPCAO_NIVEIS_APPLY=1`** confirma. Idempotente: numa reexecução insere 0 e arquiva 0.
