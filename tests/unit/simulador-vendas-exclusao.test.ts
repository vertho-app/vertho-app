...
import {
  excluirCadastroComBackupPace,
  preverExclusaoPace,
} from '@/lib/simulador-vendas/exclusao';
      ...
      sessoes: [
        {
          id: sessao,
          empresa_id: empresa,
          estado: { mensagens: [{ texto: 'privado' }] },
        },
      ],
      tentativas: [{ empresa_id: empresa, sessao_id: sessao }],
      vendas_revisoes: [
        { empresa_id: empresa, sessao_id: sessao, motivo: 'Revisão vendas' },
      ],
      lideranca_revisoes: [
        {
          empresa_id: empresa,
          jornada_id: jornada,
          motivo: 'Revisão liderança',
        },
      ],
      recepcao_sessoes: [
        {
          id: sessaoAtendimento,
          empresa_id: empresa,
          estado: { historico: [{ content: 'privado' }] },
        },
      ],
      lideranca_jornadas: [
        { id: jornada, empresa_id: empresa, estado: { plano: 'privado' } },
      ],
...
describe('PACE: exclusão administrativa com backup privado e confirmação do servidor', () => {
  ...
  beforeEach(() => {
    ...
    sb.client.rpc.mockImplementation(async (nome: string) =>
      nome === 'sim_vendas_exclusao_snapshot'
  ...
  it('a prévia pública só contém recibo opaco e contagens', async () => {
    const previa = await preverExclusaoPace(empresa, {
      tipo: 'colaborador',
      id: colaborador,
    });
  ...
  it('confirmação ausente ou inválida falha antes de consultar e gravar', async () => {
    await expect(
      excluirCadastroComBackupPace(
        empresa,
        { tipo: 'colaborador', id: colaborador },
        null,
        'admin@example.test',
      ),
  ...
  it('valida também atendimento e liderança: outro tenant ou encontro órfão reprova a prévia', async () => {
    ...
    outroTenant.documento.recepcao_sessoes[0].empresa_id =
      '10000000-0000-4000-8000-000000000099';
    sb.client.rpc.mockResolvedValueOnce({ data: outroTenant, error: null });
    await expect(
      preverExclusaoPace(empresa, { tipo: 'colaborador', id: colaborador }),
    ).rejects.toMatchObject({ status: 503 });
    const orfao = snapshot();
    orfao.documento.lideranca_episodios[0].jornada_id =
  ...
  it('valida tenant e vínculos internos do snapshot antes do upload', async () => {
    const fora = snapshot();
    fora.documento.sessoes[0].empresa_id =
      '10000000-0000-4000-8000-000000000099';
  ...
  it('bucket público, upload falho ou download divergente nunca autorizam o DELETE', async () => {
    sb.client.storage.getBucket.mockResolvedValueOnce({
      data: { public: true },
      error: null,
    });
    await expect(
    ...
    sb.client.storage.getBucket.mockResolvedValue({
      data: { public: false },
      error: null,
    });
    ...
    expect(
      sb.client.rpc.mock.calls.filter(
        ([nome]) => nome === 'sim_vendas_excluir_cadastro',
      ),
    ).toHaveLength(0);
  });
  ...
  it('backup conferido passa hash, caminho, tenant e autor à RPC', async () => {
    expect(
      await excluirCadastroComBackupPace(
        empresa,
        { tipo: 'colaborador', id: colaborador },
        hashA,
        'admin@example.test',
    ...
    const chamada = sb.client.rpc.mock.calls.find(
      ([nome]) => nome === 'sim_vendas_excluir_cadastro',
    );
    expect(chamada?.[1]).toMatchObject({
      ...
      p_hash: hashA,
      p_autor: 'admin@example.test',
      p_backup_sha256: expect.stringMatching(/^[0-9a-f]{64}$/),
      p_backup: expect.stringMatching(
        /^pace-exclusao\/\d{4}-\d{2}-\d{2}_[0-9a-f-]{36}\.json\.gz$/,
      ),
    });
  ...
  it('recusa SQL confirmada limpa o backup; timeout ambíguo o preserva', async () => {
    sb.client.rpc
      .mockImplementationOnce(async () => ({ data: snapshot(), error: null }))
      .mockResolvedValueOnce({
        data: null,
        error: { message: 'SIM_CONFIRMACAO', code: 'P0001' },
      });
    await expect(
      excluirCadastroComBackupPace(
        empresa,
    ...
    sb.client.rpc
      ...
      .mockResolvedValueOnce({
        data: null,
        error: { message: 'timeout', code: null },
      });
    await expect(
      excluirCadastroComBackupPace(
        empresa,
