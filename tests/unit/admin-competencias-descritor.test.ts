 ...
 * predicado garantem: só colunas de rubrica entram no update, o tenant está na
 * MESMA cadeia (o id vem do browser), e falha de banco volta como erro.
 */
...
vi.mock('@/lib/auth/action-context', () => ({ requirePermissionAction: vi.fn(), assertTenantAccessAction: vi.fn() }));
...
describe('salvarDescritor', () => {
  beforeEach(() => { sb.reset(); gate.mockClear(); });

  it('passa pelo gate content.manage da empresa da rota e grava só colunas de rubrica, com tenant na cadeia', async () => {
    const r = await salvarDescritor('emp-A', ID, {
      n3_meta: ' Define prioridade clara ', n4_referencia: 'Sustenta rotina', n1_gap: '', nome_curto: 'Direção',
    ...
    expect(up.payload).toEqual({ n3_meta: 'Define prioridade clara', n4_referencia: 'Sustenta rotina', n1_gap: null, nome_curto: 'Direção' });
    expect(sb.usou('competencias', 'eq', 'id')).toBe(true);
    expect(sb.usou('competencias', 'eq', 'empresa_id')).toBe(true);
  });

  it('recusa id inválido, payload sem coluna de rubrica e N3 vazio — sem escrever', async () => {
    expect(await salvarDescritor('emp-A', 'nao-e-uuid', { n3_meta: 'x' })).toMatchObject({ success: false, error: 'Descritor inválido' });
    expect(await salvarDescritor('emp-A', ID, { nome: 'x' })).toMatchObject({ success: false, error: 'Nada para salvar' });
