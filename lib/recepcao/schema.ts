...
export const rotuloClassificacao: Record<string, string> = {
  n1: 'N1 · Gap',
  n2: 'N2 · Em desenvolvimento',
  n3: 'N3 · Meta',
  n4: 'N4 · Referência',
  adequado: 'Adequado',
  parcial: 'Parcial',
  insuficiente: 'Precisa melhorar',
  nao_observavel: 'Sem oportunidade de observar',
};
export const niveisSchema = z
  .object({
    n1: texto.max(1600),
    n2: texto.max(1600),
    n3: texto.max(1600),
  ...
  .strict();
export const matrizAtendimentoSchema = z
  .object({
    versao: z.literal('atendimento-5x6-1'),
    competencias: z
      .array(
        z
          .object({
...
const paciente = z
  .object({
    postura: z.enum(['negociavel', 'resistencia_persistente']).optional(),
    nome: texto,
    abertura: texto.max(800),
    comportamento: texto,
    fatos: z.array(texto).min(1).max(12),
    limites: texto,
  })
  .strict();
...
export const rotuloNivel: Record<Nivel, string> = {
  ...
  pressao: 'Sob pressão',
  limite: 'Limite contestado',
};

export const cenarioSchema = z
  .object({
    id: chave,
    ...
    publico: z
      .object({
        ...
        aviso: texto,
        contexto: texto,
        agora: texto.optional(),
        clinica: texto.optional(),
        canal: z.enum(['mensagens', 'telefone']).default('mensagens'),
        nivel: z.enum(NIVEIS).optional(),
        escopoAvaliacao: texto.optional(),
        consultaAnterior: texto.optional(),
        alternativas: z
          .array(
            z
              .object({
          ...
          .array(
            z
              .object({
                ...
                itens: z.array(texto).min(1).max(15),
              })
              .strict(),
          )
          .max(8)
      ...
      .strict(),
    paciente,
    variantes: z.array(paciente).max(3).default([]),
    matriz: matrizAtendimentoSchema.optional(),
      ...
      .array(
        z
          .object({
            ...
            criterio: texto,
            niveis: niveisSchema.optional(),
            adequado: texto.optional(),
            parcial: texto.optional(),
            insuficiente: texto.optional(),
          })
          .strict(),
      )
      .min(3)
      .max(7),
    ocorrenciasCriticas: z.array(chave).max(12),
    desfechos: z.array(chave).min(1).max(12),
  ...
  .superRefine((c, ctx) => {
    if (c.matriz) {
      ...
      if (
        c.rubrica.length !== 5 ||
        new Set(c.matriz.competencias.map((x) => x.codigo)).size !== 5 ||
        !codigos.every(
          (k) =>
            c.matriz!.competencias.some((x) => x.codigo === k) &&
            c.rubrica.some((r) => r.id === k && r.peso === 20),
        )
      )
        ctx.addIssue({
          code: 'custom',
          path: ['matriz'],
          message: 'A matriz exige as cinco competências com o mesmo peso.',
        });
      if (
        new Set(
          c.matriz.competencias.flatMap((x) =>
            x.descritores.map((d) => d.codigo),
          ),
        ).size !== 30
      )
        ctx.addIssue({
          code: 'custom',
          path: ['matriz'],
          message: 'A matriz exige 30 descritores únicos.',
        });
    }
    if (c.rubrica.reduce((s, d) => s + d.peso, 0) !== 100)
      ctx.addIssue({
        code: 'custom',
        path: ['rubrica'],
        message: 'Os pesos devem somar 100.',
      });
    if (new Set(c.rubrica.map((d) => d.id)).size !== c.rubrica.length)
      ctx.addIssue({
        code: 'custom',
        path: ['rubrica'],
        message: 'Competências duplicadas.',
      });
    for (const [i, d] of c.rubrica.entries()) {
      ...
      if (!!d.niveis === legado)
        ctx.addIssue({
          ...
          path: ['rubrica', i],
          message:
            'Cada competência tem os quatro níveis (n1 a n4) ou os três textos legados, não ambos nem nenhum.',
        });
    }
    if (new Set(c.rubrica.map((d) => !!d.niveis)).size > 1)
      ctx.addIssue({
        code: 'custom',
        path: ['rubrica'],
...
const ref = z
  .object({
    mensagemId: z.string().max(40),
    trecho: z.string().trim().min(1).max(4000),
  })
  .strict();
export const pacienteSchema = z
  .object({ fala: z.string().trim().min(1).max(800) })
  .strict();
export const avaliacaoSchema = z
  .object({
      ...
      .array(
        z
          .object({
            ...
            justificativa: z.string().min(1).max(1600),
            evidencias: z.array(ref).max(12),
            oportunidades: z.array(ref).max(12),
          })
          .strict(),
      )
      .min(3)
    ...
    ocorrencias: z
      .array(
        z
          .object({
            categoria: chave,
            motivo: z.string().min(1).max(1600),
            evidencias: z.array(ref).min(1).max(12),
          })
          .strict(),
      )
      .max(12),
    ...
    feedback: z
      .object({
        acerto: z.string().min(1).max(1800),
        melhoria: z.string().min(1).max(1800),
        novaTentativa: z.string().min(1).max(1800),
      })
      .strict(),
  })
  .strict();
...
export const comandoSchema = z.discriminatedUnion('acao', [
  z
    .object({
      ...
      cenarioId: z.string().uuid().optional(),
    })
    .strict(),
  z
    .object({
...
export const editarCenarioSchema = z
  .object({
    ...base,
    acao: z.enum(['salvar', 'publicar', 'arquivar']),
    id: z.string().uuid().optional(),
    revisao: z.number().int().nonnegative().optional(),
    conteudo: cenarioSchema.optional(),
...
export const competenciaSchema = z
  .object({
    codigo: chave,
    nome: texto.max(120),
    descricao: z.string().trim().max(1600).default(''),
    niveis: niveisSchema,
  })
  .strict();
export const competenciaComandoSchema = z
  .object({
    ...base,
    acao: z.literal('competencia'),
    op: z.enum(['salvar', 'excluir', 'restaurar']),
    id: z.string().uuid().optional(),
    revisao: z.number().int().nonnegative().optional(),
