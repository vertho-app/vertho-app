...
SELECT
  ...
  s.score_contexto_setorial, s.commercial_actionability, s.head_est,
...
COPY (
  WITH ativ AS (
    ...
    -- head_est (híbrido RAIS→porte) já vem do Stage 4 por linha — aqui
    -- só agrega (sem join/regexp → memória mínima, evita o OOM).
