...
import { agregarPainel, MAX_COMENTARIOS, type SessaoPainel } from '@/lib/simulador-vendas/painel';
...
const colaboradores = [
  { id: 'ana', nome_completo: 'Ana', cargo: 'Vendedor', email: 'ana@cliente.test', role: 'colaborador', gestor_email: 'gestor@cliente.test' },
  { id: 'bia', nome_completo: 'Bia', cargo: 'Vendedor', email: 'bia@cliente.test', role: 'colaborador', gestor_email: 'outro@cliente.test' },
  { id: 'caio', nome_completo: 'Caio', cargo: 'Financeiro', email: 'caio@cliente.test', role: 'colaborador', gestor_email: 'gestor@cliente.test' },
  { id: 'davi', nome_completo: 'Davi', cargo: 'Vendedor', email: 'davi@cliente.test', role: 'colaborador', gestor_email: 'gestor@cliente.test' },
  { id: 'gil', nome_completo: 'Gil', cargo: 'Vendedor', email: 'gestor@cliente.test', role: 'gestor', gestor_email: null },
  { id: 'rute', nome_completo: 'Rute', cargo: 'Vendedor', email: 'rh@cliente.test', role: 'rh', gestor_email: null },
  { id: 'suporte', nome_completo: 'Suporte', cargo: 'Vendedor', email: 'suporte@vertho.ai', role: 'colaborador', gestor_email: 'gestor@cliente.test' },
].map((p) => ({ ...p, empresa_id: empresa }));
const outroTenant = { id: 'fora', empresa_id: 'empresa-b', nome_completo: 'Fora', cargo: 'Vendedor', email: 'f@b.test', role: 'colaborador', gestor_email: 'gestor@cliente.test' };

const sessoes = [
  {
    id: 's1', colaborador_id: 'ana', created_at: '2026-09-10T12:00:00Z',
    resumo: { status: 'concluida', versaoRegua: 'pace-7' },
    pl: 2.5, p: 2.6, a: 3.1, c: null, e: 3.2,
    feedback: { realismo: 4, desafio: 3, interacao: 4, utilidade: 5, aprendizado: 4, comentario: 'Faltou pressão no preço.' },
  },
  {
    id: 's2', colaborador_id: 'ana', created_at: '2026-09-12T12:00:00Z',
    resumo: { status: 'concluida', versaoRegua: 'pace-7' },
    pl: 3.2, p: 3.0, a: 2.4, c: 3.0, e: 3.1,
    feedback: null,
  },
  {
    id: 's3', colaborador_id: 'davi', created_at: '2026-09-11T12:00:00Z',
    resumo: { status: 'concluida', versaoRegua: 'pace-5' },
    pl: null, p: 7.5, a: 7.5, c: 7.5, e: 7.5,
    feedback: { realismo: 5, desafio: 5, interacao: 5, utilidade: 5, aprendizado: 5, comentario: '' },
  },
];
...
const c = (role = 'gestor', admin = false) =>
  ({
    ...
    auth: {
      ...
      empresaId: empresa,
      colaborador: { id: 'gil', empresa_id: empresa, email: 'gestor@cliente.test' },
    },
...
function mock(sysConfig: Record<string, unknown> | null = null) {
  sb = criarSupabaseMock({
    ...
    lista: (t, _cols, cadeia) => {
      if (t === 'cargos_empresa') return [{ id: 'cargo-vendas', nome: 'Vendedor' }, { id: 'cargo-fin', nome: 'Financeiro' }];
      // O tenantDb filtra por empresa no banco; aqui o filtro vem da própria cadeia.
      if (t === 'colaboradores')
        return [...colaboradores, outroTenant].filter((p) => p.empresa_id === valorDe(cadeia, 'eq', 'empresa_id'));
...
describe('painelEquipe: população, escopo e leitura', () => {
  beforeEach(() => {
    mock({ simuladores_por_cargo: { 'cargo-vendas': { vendas: true }, 'cargo-fin': { vendas: false } } });
  ...
  it('gestor vê os liderados com o cargo liberado; gestor, RH e contas internas não entram', async () => {
    ...
    expect(painel.resumo).toEqual({ pessoas: 2, comecaram: 2, concluiram: 2, naoComecaram: 0 });
  });
  ...
  it('treino na escala antiga (0 a 10) não entra no nível; a pesquisa dele entra', async () => {
    ...
    expect(davi.competencias.every((x) => x.nivelAlcancado === null)).toBe(true);
...
describe('agregarPainel', () => {
  ...
  const s = (colaboradorId: string, criadoEm: string, status: string, extra: Partial<SessaoPainel> = {}): SessaoPainel => ({
    colaboradorId,
    criadoEm,
    status,
    competencias: null,
  ...
  it('descartado não conta como treino; em andamento conta e aparece', () => {
    ...
    expect(p.pessoas.find((x) => x.id === 'b')).toMatchObject({ treinos: 1, concluidos: 0, emAndamento: true });
  });
  ...
  it('distribuição por competência usa o maior nível de cada pessoa', () => {
    ...
    expect(p.pessoas.find((x) => x.id === 'a')!.competencias.find((x) => x.codigo === 'P')!.subiu).toBe(true);
  });
  ...
  it('pesquisa: médias ignoram nota inválida; comentários sem nome, mais recentes primeiro e com teto', () => {
    const muitas = Array.from({ length: MAX_COMENTARIOS + 5 }, (_, i) =>
      s('a', `2026-09-${String((i % 28) + 1).padStart(2, '0')}T${String(i % 24).padStart(2, '0')}:00:00Z`, 'concluida', {
        feedback: { realismo: i === 0 ? 9 : 4, comentario: `Comentário ${i}` },
      }),
    );
    ...
    expect(Object.keys(p.pesquisa.comentarios[0]).sort()).toEqual(['em', 'texto']);
