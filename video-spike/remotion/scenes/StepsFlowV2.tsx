      ...
      <AbsoluteFill>
        ...
        <AbsoluteFill style={{ flexDirection: 'column', justifyContent: 'center', padding: '0 168px', opacity: out }}>
          ...
          <div style={{ position: 'relative' }}>
            {/* linha-acento vertical que se desenha à esquerda dos índices */}
            ...
            {steps.map((s, i) => {
              ...
              return (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 44, padding: '24px 0 24px 56px', borderBottom: last ? 'none' : `1px solid ${withAlpha('#ffffff', 0.07)}`, opacity: p, transform: `translateX(${(1 - p) * 24}px)` }}>
                  <span style={{ flex: '0 0 auto', width: 116, fontSize: 100, fontWeight: 800, lineHeight: 0.8, letterSpacing: -3, color: withAlpha(brand.primary, 0.26), fontVariantNumeric: 'tabular-nums' }}>{String(i + 1).padStart(2, '0')}</span>
    ...
    <AbsoluteFill>
      ...
      <AbsoluteFill style={{ flexDirection: 'column', justifyContent: 'center', padding: '0 140px', opacity: out }}>
        ...
        <div style={{ position: 'relative', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          {/* trilho de progresso ALINHADO ao centro dos numerais (top 46) */}
          ...
          {steps.map((s, i) => {
            ...
            return (
              <div key={i} style={{ position: 'relative', width: `${100 / steps.length}%`, display: 'flex', flexDirection: 'column', alignItems: 'center', opacity: p, transform: `translateY(${(1 - p) * 22}px)` }}>
                {/* numeral fantasma com um chip sólido pequeno atrás cobrindo o trilho */}
                ...
                <span style={{ fontSize: 86, fontWeight: 800, lineHeight: 0.9, letterSpacing: -3, color: withAlpha(brand.primary, 0.24), fontVariantNumeric: 'tabular-nums', marginTop: 22 }}>{String(i + 1).padStart(2, '0')}</span>
