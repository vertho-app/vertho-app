...
export async function parsePdf(buffer: Buffer): Promise<ParsedDoc> {
  // pdfjs-dist v5 legacy ainda pode pedir DOMMatrix em Node — polyfill mínimo.
  if (typeof globalThis.DOMMatrix === 'undefined') {
    const { DOMMatrix: Polyfill } = await import('@aspect-build/dommatrix' as any).catch(() => {
      // Fallback inline se o pacote não estiver instalado
    ...
    (globalThis as any).DOMMatrix = Polyfill;
  }
