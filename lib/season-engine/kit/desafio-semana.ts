...
export async function resolverDesafioDoKit(
  ...
  let q = sb.from('kit_briefs').select('id, empresa_id').eq('competencia', args.competencia).eq('descritor', args.descritor);
  q = args.empresaId ? q.or(`empresa_id.eq.${args.empresaId},empresa_id.is.null`) : q.is('empresa_id', null);
  const { data: briefs } = await q;
  if (!briefs?.length) return null;

  // Prefere o brief EXCLUSIVO da empresa sobre o global.
  briefs.sort((a: any, b: any) => (b.empresa_id ? 1 : 0) - (a.empresa_id ? 1 : 0));
