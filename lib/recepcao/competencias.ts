...
export async function editarCompetencia(
  ...
  if (!cmd.id) {
    if (cmd.op !== 'salvar' || !cmd.conteudo)
      throw new RecepcaoError(400, 'Informe a competência a criar.');
    const conteudo = competenciaSchema.parse(cmd.conteudo);
    const { data, error } = await c.sb
      .from('recepcao_competencias')
      .insert({
        id: randomUUID(),
        ...conteudo,
        ativo: true,
    ...
    if (error?.code === '23505')
      throw new RecepcaoError(
        409,
        'Já existe uma competência com este código.',
      );
    if (error)
      throw new RecepcaoError(503, 'Não foi possível criar a competência.');
    return data;
  }
  const { data: atual, error: readError } = await c.sb
    .from('recepcao_competencias')
    .select('*')
    .eq('id', cmd.id)
    .maybeSingle();
  if (readError)
    throw new RecepcaoError(503, 'Não foi possível consultar a competência.');
  if (!atual) throw new RecepcaoError(404, 'Competência não encontrada.');
  if (cmd.revisao !== atual.revisao)
    throw new RecepcaoError(
      409,
      'A competência mudou. Recarregue antes de editar.',
    );
  let patch: Record<string, unknown>;
  if (cmd.op === 'salvar') {
    if (!cmd.conteudo)
      throw new RecepcaoError(400, 'Informe o conteúdo da competência.');
    const conteudo = competenciaSchema.parse(cmd.conteudo);
    if (conteudo.codigo !== atual.codigo)
      throw new RecepcaoError(
        400,
        'O código não muda depois de criado; crie outra competência.',
      );
    patch = {
      nome: conteudo.nome,
      descricao: conteudo.descricao,
      niveis: conteudo.niveis,
    };
  ...
  const { data, error } = await c.sb
    ...
    .maybeSingle();
  if (error)
    throw new RecepcaoError(503, 'Não foi possível salvar a competência.');
  if (!data)
    throw new RecepcaoError(
      409,
      'A competência mudou durante a gravação. Recarregue.',
    );
  return data;
}
