...
import { competenciasDaJornada, contextoEquipe, detalhePessoa, painelEquipe, revisarJornada } from '@/lib/simulador-lideranca/equipe';
...
const revisoesGravadas = [
  { id: 'r1', parecer: 'discordo', motivo: 'A análise foi melhor do que a nota.', dimensoes: [], revisor_nome: 'Rute', created_at: '2026-09-18T15:00:00Z' },
];
function banco() {
    ...
    resolver: (tabela, _cols, cadeia) => {
      ...
      if (tabela === 'sim_lideranca_jornadas') {
        ...
        return eq('colaborador_id') === 'ana' || eq('id') === 'j-ana'
          ? {
              id: 'j-ana', colaborador_id: 'ana', updated_at: '2026-09-18T12:00:00Z',
              estado: { ...estado(), concluidos: semDevolutiva ? [] : [concluido] },
            }
          : null;
      }
...
describe('revisão humana da liderança (18/09/2026, mig 264)', () => {
  ...
  const comando = (extra: Record<string, unknown> = {}) =>
    ({ acao: 'revisar', requestId: REQ, alvoId: 'j-ana', parecer: 'discordo', motivo: 'A análise foi melhor do que a nota.', dimensoes: [], ...extra }) as any;
  const gestor = () => {
    const a = auth('gestor');
  ...
  it('o detalhe traz a jornada, as revisões, quem pode revisar e as competências da matriz', async () => {
    ...
    expect(d).toMatchObject({ jornadaId: 'j-ana', revisoes: revisoesGravadas, podeRevisar: true });
  ...
  it('grava o parecer na jornada, com a chave e o nome de quem revisa', async () => {
    ...
    expect(await revisarJornada(await contextoEquipe(gestor()), comando({ dimensoes: [foco] }))).toEqual({ ok: true });
    const w = sb.escritas.find((e) => e.tabela === 'sim_lideranca_revisoes')!;
    expect(w.payload).toMatchObject({
      id: REQ, empresa_id: EMP, jornada_id: 'j-ana', revisor_key: 'colab:eu', revisor_nome: 'Gil Gestor',
      parecer: 'discordo', dimensoes: [foco],
    });
  });

  it('administrador da plataforma revisa com a chave do cadastro dele na plataforma', async () => {
    sb = banco();
    const admin = { ...auth('rh', 'suporte@vertho.ai'), isPlatformAdmin: true, colaborador: null };
    expect(await revisarJornada(await contextoEquipe(admin, EMP), comando())).toEqual({ ok: true });
    expect(sb.escritas.find((e) => e.tabela === 'sim_lideranca_revisoes')!.payload).toMatchObject({
      revisor_key: 'admin:adm-1', revisor_nome: 'Administração Vertho',
    });
  });

  it('🔴 a própria pessoa não revisa a sua jornada', async () => {
    sb = banco();
    const ana = { ...auth('gestor', 'ana@x.test'), colaborador: { id: 'ana', empresa_id: EMP, email: 'ana@x.test' } };
    expect((await detalhePessoa(await contextoEquipe(ana), 'ana')).podeRevisar).toBe(false);
    await expect(revisarJornada(await contextoEquipe(ana), comando())).rejects.toMatchObject({ status: 403 });
  ...
  it('jornada sem devolutiva ainda não recebe revisão', async () => {
    ...
    try {
      sb = banco();
      expect((await detalhePessoa(await contextoEquipe(gestor()), 'ana')).podeRevisar).toBe(false);
      await expect(revisarJornada(await contextoEquipe(gestor()), comando())).rejects.toMatchObject({ status: 409 });
      expect(sb.escritas).toHaveLength(0);
    } finally {
      semDevolutiva = false;
    }
  ...
  it('fora da equipe, sem permissão de registro ou com competência de fora: nada grava', async () => {
    ...
    const outraEquipe = { ...gestor(), email: 'outro@x.test', colaborador: { id: 'eu', empresa_id: EMP, email: 'outro@x.test' } };
    await expect(revisarJornada(await contextoEquipe(outraEquipe), comando())).rejects.toMatchObject({ status: 404 });
    await expect(revisarJornada(await contextoEquipe(gestor()), comando({ dimensoes: ['NAO_EXISTE'] }))).rejects.toMatchObject({ status: 400 });
    vi.mocked(can).mockImplementation(async (_a, p) => p !== 'assessments.answer');
    try {
      await expect(revisarJornada(await contextoEquipe(gestor()), comando())).rejects.toMatchObject({ status: 403 });
    } finally {
