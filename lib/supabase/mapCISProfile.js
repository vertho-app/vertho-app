 ...
 *
 * Várias colunas (índices e tipo psicológico numérico) podem ainda não estar
 * preenchidas no banco — quando não estão, o mapper aplica fallbacks razoáveis
 * para que o relatório consiga ser gerado mesmo num mapeamento legado.
 */
...
export function mapSupabaseToCISRawData(row) {
  ...
  return {
    ...
    competencias: COMPETENCIAS.map(([nome, base]) => ({ nome, natural: num(row[base], 0) })),
  };
}
...
export const CIS_COLUMNS = [
  ...
  // 16 competências
  ...COMPETENCIAS.map(([, base]) => base),
  // Cache
  'report_texts', 'report_generated_at', 'comportamental_pdf_path',
