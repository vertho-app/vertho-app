...
export function construirEvolucao(
  ...
  const descritores: DescritorEvolucao[] = descritoresDaVitrine(
    ...
    .map((descritor) => {
      ...
      // O perfil "confirmada" chega a N3 de propósito, para a vitrine mostrar
      // evolução com patamar. A régua não exige mais o N3 para confirmar
      // (17/09/2026); o piso ficou só como escolha de roteiro da demo.
      const bruto = nota_pre + ganhoDoPerfil(perfil, seed);
      const nota_pos = arredondar(perfil === 'confirmada'
        ? Math.max(bruto, 3 + ((seed % 3) / 10))
        : bruto);
