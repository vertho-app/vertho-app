...
function linhaColab(p: PessoaAdequacao): string {
  // DISC = só valores brutos, SEM marca (dentro)/(FORA). A marca por valor bruto
  // induzia a IA a citar traços quase-na-faixa (ex.: C=40, fit ~99%) só pra negá-los
  // ("fora da faixa mas não conta") — reincidência do B1. Atenção vem SÓ dos DRIVERS
  // (fit abaixo do alvo); o resto é contexto neutro e não deve virar texto.
