    ...
    <div className="max-w-[1000px] mx-auto px-4 py-6 sm:px-6" style={{ minHeight: '100dvh' }}>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-white flex items-center gap-2">
              <List size={20} className="text-cyan-400" /> Listas de prospecção
            </h1>
            <p className="text-xs text-gray-500">Recortes salvos · export ordenado por priority_rank</p>
          </div>
        </div>
      ...
      {criando && (
        <div className="mb-5 p-4 rounded-xl border border-cyan-400/20" style={{ background: '#0F2A4A' }}>
          <div className="flex items-center justify-between mb-3">
            <p className="text-xs font-bold text-white">Nova lista</p>
            <button onClick={() => { setCriando(false); setNome(''); setDesc(''); }} className="text-gray-500 hover:text-white"><X size={14} /></button>
          </div>
          <input value={nome} onChange={e => setNome(e.target.value)} placeholder="Nome (ex: Saúde Jundiaí — Q3)"
          ...
          <p className="text-[9px] text-gray-500 mt-2">Adicione empresas pela busca (botão "+ lista" na ficha — em breve) ou exporte direto da busca com filtro.</p>
        </div>
      )}
      ...
      {listas.length === 0 ? (
        <div className="text-center py-16">
          ...
          <p className="text-sm text-gray-500">Nenhuma lista ainda. Crie uma ou exporte direto da busca.</p>
        </div>
      ) : (
            ...
            <div key={l.id} className="flex items-center justify-between p-4 rounded-xl border border-white/[0.06]" style={{ background: '#0F2A4A' }}>
              <div>
                ...
                <p className="text-[10px] text-gray-600 mt-1">
                  {l.total_itens} item(s) · {Object.entries(l.por_status || {}).map(([s, n]) => `${s}: ${n}`).join(' · ') || 'sem itens'}
                </p>
              </div>
