...
export async function gerarRelatorioIndividual(empresaId, colaboradorId, aiConfig = {}) {
  ...
  try {
    ...
    const resultado = await callAI(RELATORIO_IND_SYSTEM, user, aiConfig, 16000);
...
export async function gerarRelatorioGestor(empresaId, aiConfig = {}) {
  ...
  try {
    ...
    const resultado = await callAI(RELATORIO_GESTOR_SYSTEM, user, aiConfig, 16000);
    const relatorio = await extractJSON(resultado);
...
${JSON.stringify(registros, null, 2)}`;
    ...
    const resultado = await callAI(RELATORIO_RH_SYSTEM, user, aiConfig, 16000);
    const relatorio = await extractJSON(resultado);
