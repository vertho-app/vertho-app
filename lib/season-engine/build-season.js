 ...
 *   - 30% taxa_conclusao (já 0-1)
 * Com menos de 5 amostras de impacto, usa só taxa_conclusao (dado insuficiente).
 * Retorna 0 se não há nenhum sinal.
 */
function computarScoreConteudo(c) {
  ...
  for (let semana = 1; semana <= 14; semana++) {
    ...
    if ([4, 8, 12].includes(semana)) {
      ...
      if (!d) {
        ...
        if (plan.conteudo?.core_id) idsJaUsados.add(plan.conteudo.core_id);
      }
    }
...
async function montarSemanaConteudo(semana, descritorSel, competencia, cargo, contexto, prioridadeFormatos, empresaId, aiConfig, idsJaUsados = new Set()) {
  const sb = createSupabaseAdmin();
  const nivelMedio = (descritorSel.nota_atual + 3.0) / 2;

  // Busca conteúdos pra esse descritor (com fallback gradual via or).
  // Tenta primeiro com filtro de nivel; se vazio (ou só 1 item), busca
  // todos da competência sem restrição de nivel para ter pool maior e
  // evitar duplicação entre semanas.
  const buildQ = (withNivel) => {
  ...
  const todosComp = candidatos || [];
  // Cascata: (1) match descritor não usado → (2) qualquer da competência não
  // usado → (3) pool original (aceita repetir como último recurso).
  ...
  const pool = poolDescDisp.length > 0
    ? poolDescDisp
  ...
  //   score = 0.7 * impacto_medio_delta (normalizado) + 0.3 * taxa_conclusao
  // Impacto só conta se amostras >= 5 (senão é ruído). Sem amostras,
  // cai pra taxa_conclusao. Fecha o loop: conteúdo popular mas inefetivo
  // perde pra conteúdo que gera evolução real nos descritores.
  ...
  // Marca explicitamente quando reutiliza um conteúdo já consumido —
  // útil para a UI sinalizar "continuação" em vez de aparentar bug.
...
async function montarSemanaAplicacao(semana, descritores, competencia, cargo, contexto, aiConfig) {
  ...
  // Gera missão prática (default) e cenário escrito (fallback) em paralelo.
  let missaoTexto = '';
  let cenarioTexto = '';
  try {
    ...
    console.warn(`[buildSeason] missao/cenario sem ${semana}: ${err.message}`);
  ...
  return {
    ...
    cenario: { texto: cenarioTexto, complexidade }, // fallback
    status: 'bloqueada',
  };
}
