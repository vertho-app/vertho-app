            ...
            try {
              if (params.descritor) {
                ...
                if (descritores.length === 0) {
                  addLog(`❌ Nenhum descritor pra "${params.competencia}"`, 'error');
                  return;
                }
                addLog(`📋 Vai gerar ${descritores.length} ${params.formato}(s) pra "${params.competencia}"`, 'info');
                let ok = 0, erros = 0;
                ...
                addLog(`🎉 Lote concluído: ${ok}/${descritores.length}${erros ? ` (${erros} erros)` : ''}`, ok === descritores.length ? 'success' : 'info');
                setShowGerar(false);
              }
            } catch (e) {
...
function SelectField({ label, value, onChange, options, disabled, name, defaultValue }: { label?: any; value?: any; onChange?: any; options: any[]; disabled?: any; name?: any; defaultValue?: any }) {
  ...
  return (
    <div>
      ...
      >
        {options.map((o: any) => <option key={o} value={o} className="bg-[#0d1426] text-white">{o || '— selecione —'}</option>)}
      </select>
    </div>
  );
}
