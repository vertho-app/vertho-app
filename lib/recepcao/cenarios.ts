...
export async function catalogo(c: ContextoRecepcao, editor = false) {
  ...
  const { data, error } = await q
    ...
    .limit(200);
  if (error)
    throw new RecepcaoError(503, 'Não foi possível carregar os cenários.');
  if (editor) return data || [];
  // Para quem treina: degraus em ordem (introdução → sob pressão → limite contestado), título dentro do degrau.
  return ordenarPorNivel(
    (data || []).map((r) => ({
      id: r.id,
      versao: r.versao,
      ficha: fichaPublica(cenarioSchema.parse(r.conteudo)),
    })),
  );
}
export async function cenarioPublicado(
  c: ContextoRecepcao,
  id?: string,
): Promise<{ id: string; conteudo: Cenario }> {
  let q = c.sb
    .from('recepcao_cenarios')
    .select('id,conteudo')
    .or(`empresa_id.eq.${c.empresaId},empresa_id.is.null`)
  ...
  const { data, error } = await q
    ...
    .maybeSingle();
  if (error)
    throw new RecepcaoError(503, 'Não foi possível consultar o cenário.');
  if (!data)
    throw new RecepcaoError(
      404,
      'Este cenário não está disponível para iniciar. Selecione outro caso.',
    );
  return { id: data.id, conteudo: cenarioSchema.parse(data.conteudo) };
}
export async function editarCenario(
  c: ContextoRecepcao,
  ...
  if (cmd.catalogo && !c.auth.isPlatformAdmin)
    throw new RecepcaoError(403, 'Só a plataforma edita o Catálogo Vertho.');
  const hashRubrica = (c: Cenario) =>
    createHash('sha256')
  ...
  if (!cmd.id) {
    ...
    const id = randomUUID(),
      conteudo = cenarioSchema.parse(
        aplicarMatrizAtendimento(cenarioSchema.parse(cmd.conteudo)),
      );
    ...
    conteudo.rubricaVersao = hashRubrica(conteudo);
    const { data, error } = await c.sb
      .from('recepcao_cenarios')
      .insert({
        id,
        empresa_id: cmd.catalogo ? null : c.empresaId,
        codigo: conteudo.id,
        versao: conteudo.versao,
        conteudo,
        created_by: c.ownerKey,
      })
      .select('*')
      .single();
    if (error?.code === '23505')
      throw new RecepcaoError(
        409,
        'Já existe essa versão deste caso. Informe outra versão.',
      );
    if (error)
      throw new RecepcaoError(503, 'Não foi possível criar o rascunho.');
    return data;
  }
  ...
  if (readError)
    throw new RecepcaoError(503, 'Não foi possível consultar o rascunho.');
  if (!atual)
    throw new RecepcaoError(
      404,
  ...
  if (cmd.revisao !== atual.revisao)
    throw new RecepcaoError(
      409,
      'O cenário mudou. Recarregue antes de editar.',
    );
  if (atual.estado !== 'rascunho' && cmd.acao !== 'arquivar')
    throw new RecepcaoError(
      409,
      'Crie uma nova versão para alterar um cenário publicado.',
    );
  if (atual.estado === 'arquivado')
    throw new RecepcaoError(409, 'Esta versão já foi arquivada.');
  const global = atual.empresa_id === null;
  const conteudo = cenarioSchema.parse(
    cmd.acao === 'arquivar'
      ? atual.conteudo
      : aplicarMatrizAtendimento(
          cenarioSchema.parse(cmd.conteudo || atual.conteudo),
        ),
  );
  if (cmd.acao === 'arquivar' && cmd.conteudo)
    throw new RecepcaoError(400, 'Arquivar não altera o conteúdo.');
  ...
  const payload =
    cmd.acao === 'arquivar'
      ? {}
      : { conteudo, codigo: conteudo.id, versao: conteudo.versao };
  let escrita = c.sb
    .from('recepcao_cenarios')
    .update({
      ...payload,
    ...
    .eq('id', cmd.id)
    .eq('revisao', cmd.revisao);
  escrita = global
    ? escrita.is('empresa_id', null)
    : escrita.eq('empresa_id', c.empresaId);
  ...
  if (global && cmd.acao === 'publicar') {
    ...
    if (arquivar)
      throw new RecepcaoError(
        503,
        'A versão foi publicada, mas a anterior do catálogo não pôde ser arquivada. Arquive-a na biblioteca.',
      );
  }
  return data;
}
