 ...
 *  6. **18/08, minutos depois: entrar sozinho, mas NO NAVEGADOR DO APARELHO.**
 *     O dono testou e trouxe o que faltava: *"foi direto, mas para o navegador
 *     interno do WA"*. Zero toque e sessão presa ao WebView era metade do
 *     prêmio — o Android já saía para o Chrome (`intent://`, no `/entrar`) e o
 *     iPhone não saía para lugar nenhum. Agora o iPhone tenta o Safari sozinho
 *     (`x-safari-https://`), e o que era o botão do `<details>` virou o
 *     comportamento padrão, **sem custar o toque de volta**.
 *
 * ⚠️ O ciclo 2→6 é a lição inteira em miniatura: o mecanismo (sair para o
 * navegador) estava construído e testado desde 15/08 e passou por principal,
 * escondido e principal de novo. O que mudou nunca foi o mecanismo — foi **para
 * quem** e **a que preço**. Enquanto custava um toque de todos, valia esconder;
 * de graça, é o melhor destino.
 *
...
export default async function ConfirmarAcesso({
  ...
  //
  // Nada aqui é revelado por JavaScript. Se o `x-safari-https://` não for
  // repassado (ele não é API suportada — pode parar numa atualização do
  // WhatsApp), a navegação é simplesmente cancelada e a pessoa continua nesta
  // tela, que já traz os dois caminhos visíveis. Esconder o fallback atrás de um
  // timer seria construir exatamente a falha silenciosa que este fluxo evita.
  if (sairParaSafari) {
    return (
      <main className="flex min-h-dvh flex-col justify-center bg-[#061526] px-6 py-10 text-white">
        <div className="mx-auto w-full max-w-md">
  ...
  return (
    <main className="flex min-h-dvh flex-col justify-center bg-[#061526] px-6 py-10 text-white">
      <div className="mx-auto w-full max-w-md">
        ...
        {embutido ? (
          // Android caído no fallback do `intent://` (Chrome desativado), ou um
          // WebView que não é iOS. A saída manual continua aqui — no Android ela
          // já foi tentada automaticamente antes desta tela.
