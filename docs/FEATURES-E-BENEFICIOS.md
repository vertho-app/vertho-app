...
| **Engajamento da trilha (telemetria)** *(jul/2026)* | Visão atual em `/admin/engajamento` + página longitudinal em `/admin/engajamento/evolucao`: ativação, consumo e evidência por semana, trajetórias, recuperados, heatmap por área e fila operacional de risco | Distingue "recebeu" de "engajou" e mostra a direção da jornada — RH identifica queda, recuperação e áreas que precisam de apoio. |
| **Índice operacional transparente** | Ativação 20 + consumo 30 + evidência 40 + Tira-Dúvidas 10; denominador semanal considera apenas quem alcançou a semana | Permite acompanhar ritmo com uma régua explicável, sem vender o indicador como nota, competência ou desempenho individual. |
