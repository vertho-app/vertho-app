...
- **Output estruturado**: JSON com `intencao`, `tom_usuario` (`neutro|irritado|ofensivo|sofrimento|denuncia`), `continua_escalada` (a mensagem insiste no assunto que o Beto já passou para a equipe; só vale com `CONTEXTO.aguardando_equipe=true`), `solicita_link`, `resposta`, `precisa_humano` e `acao`.
