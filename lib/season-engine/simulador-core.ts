/**
 * NÚCLEO SEM GATE do simulador de temporada (padrão CLAUDE.md: headless →
 * núcleo em lib/, fora de 'use server'). A action `actions/simulador-temporada.ts`
 * aplica o gate de admin e delega aqui; scripts de medição (piloto de custo)
 * chamam direto com um client service-role. Recebe `sbRaw` (service-role) já
 * pronto — NÃO autentica.
 */
...
/**
 * Opções de ledger para TODA chamada originada no simulador: marca
 * source='simulator' (isola a rodada de medição do tráfego real) + atribui
 * empresa/colaborador + taskKey do fluxo. Chamadas-mentor usam o taskKey real
 * do fluxo (contam como custo representativo); o aluno usa 'sim_aluno' (netável).
 */
function simOpts(trilha: any, taskKey: string) {
  return {
    source: 'simulator',
    taskKey,
 ...
 */
export interface SimUmaSemanaParams {
  trilhaId: string;
...
export async function simularUmaSemanaCore(sbRaw: any, { trilhaId, semana, perfilEvolucao = 'evolucao_parcial', mentorModel = MENTOR_MODEL_DEFAULT }: SimUmaSemanaParams) {
...
export async function simularTemporadaCore(sbRaw: any, { trilhaId, perfilEvolucao = 'evolucao_parcial', mentorModel = MENTOR_MODEL_DEFAULT }: { trilhaId: string; perfilEvolucao?: string; mentorModel?: string }) {
  const { data: trilha } = await sbRaw.from('trilhas')
    .select('id, empresa_id, colaborador_id, competencia_foco, temporada_plano, descritores_selecionados')
    .eq('id', trilhaId).maybeSingle();
  if (!trilha) return { error: 'Trilha não encontrada' };
...
- NÃO infle qualidade sem sustentação`;
    const r = await callAI(SIM_EXTRACTOR_SYSTEM, usr, { model: mentorModel }, 2000, simOpts(trilha, 'sim_extracao_socratic'));
    let cleaned = r.trim();
...
- NÃO force todas as notas altas`;
    const r = await callAI(SIM_EXTRACTOR_SYSTEM, usr, { model: mentorModel }, 3000, simOpts(trilha, 'sim_extracao_missao'));
    let cleanedA = r.trim();
