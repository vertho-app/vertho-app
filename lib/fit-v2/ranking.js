...
export function gerarRanking(resultados, blocosCriticos = []) {
  ...
  items.sort((a, b) => {
    // 0. Reprovados por eliminatória vão SEMPRE pro fim (mesmo com match alto).
...
export function gerarDistribuicao(resultadosComRanking) {
  ...
  for (const r of resultadosComRanking) {
    // Reprovado por eliminatória conta como crítica (independe do match).
    const faixa = r.knockout_failed ? 'critica' : getFaixaDistribuicao(r.fit_final);
    dist[faixa]++;
  }
  return dist;
}
