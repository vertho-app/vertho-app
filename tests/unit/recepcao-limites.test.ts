...
test.each(catalogoLimites)('$id: resistência persistente respeita schema, isolamento e versionamento',c=>{
 expect(cenarioSchema.safeParse(c).success).toBe(true);expect(c.versao).toBe('3.2');
 for(let v=0;v<2;v++){
 ...
 expect(anterior.versao).toBe('2.2');expect(promptPaciente(anterior)).toContain('Reduza a resistência quando');
});
