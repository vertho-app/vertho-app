...
function LeadershipPie({ data }: { data: any }) {
  const segments = [
  ...
  const total = segments.reduce((acc: number, s: any) => acc + (s.value || 0), 0) || 100;
  const cx = 60;
  const cy = 60;
  const r = 50;
  let cumulative = 0;
...
// ============================================================
function Page3({ raw, texts }: { raw: any; texts: any }) {
    ...
    <PageFrame pageNum={3}>
      ...
      <View style={{ flexDirection: 'row' }}>
        <View style={{ flex: 1, marginRight: 6 }}>
          ...
          {(texts.top5_forcas || []).map((f: any, i: number) => (
            <View key={i} style={{
        ...
        <View style={{ flex: 1, marginLeft: 6 }}>
          ...
          {(texts.top5_desenvolver || []).map((d: any, i: number) => (
            <View key={i} style={{
...
// ============================================================
function Page4({ raw, texts }: { raw: any; texts: any }) {
  return (
    <PageFrame pageNum={4}>
...
// ============================================================
function Page5({ raw, texts }: { raw: any; texts: any }) {
...
// ============================================================
function PageCompetenciasCompletas({ raw }: { raw: any }) {
  ...
  return (
    <PageFrame>
      ...
      <View style={{ flexDirection: 'row', flexWrap: 'wrap' }}>
        {['D', 'I', 'S', 'C'].map((dim: string, idx: number) => {
          const grupo = (GRUPOS as any)[dim];
          const c = (DISC as any)[dim];
          return (
            <View key={dim} style={{
              width: '48%',
