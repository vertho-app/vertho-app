...
export async function setEmpresaFonteExterna(
  ...
  const sb = await requireEmpresaSupabaseStrict(empresaId, 'settings.company.manage', 'perfil_externo.fonte');
...
export async function uploadPerfilPdf(
  ...
  const sb = await requireEmpresaSupabaseStrict(empresaId, 'content.manage', 'perfil_externo.upload_pdf');
