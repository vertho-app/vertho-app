...
export async function masterizarAudio({ videoIn, bedRespiro, videoOut }) {
  // mix (voz+SFX) + bed com berço acústico + ducking sidechain — base comum aos 2 passes.
  const base = [
  ...
  // PASS 2 — aplica: two-pass linear (crava -14 exato) se mediu; senão single-pass.
  const ln = measured
  ...
  const aplicar = [...base, `[m]${ln}[a]`].join(';');
  await exec(FFMPEG, [
    '-y', ...inputs,
