...
- Name: mapeamento-disc.spec.js >> Mapeamento DISC >> completes rank phase 2 (8 groups)
- Location: tests\mapeamento-disc.spec.js:158:3
    ...
    - main [ref=e10]:
      - generic [ref=e11]:
        ...
        - generic [ref=e27]:
          ...
          - generic [ref=e48]:
            - generic [ref=e49]: "3"
            - generic [ref=e50]: Direcionador(a)
            - generic [ref=e51]:
              - button [ref=e52]:
                - img [ref=e53]
              - button [ref=e55]:
...
> 165 |     await expect(page.locator('text=Adaptado')).toBeVisible({ timeout: 5000 });
  ...
  255 | 
  256 |     // Rate all formats
  257 |     const formatRows = page.locator('.space-y-2 > div').filter({ has: page.locator('button:has-text("★")') });
  258 |     const count = await formatRows.count();
  259 |     for (let i = 0; i < count; i++) {
  260 |       await formatRows.nth(i).locator('button:has-text("★")').nth(2).click();
  261 |       await page.waitForTimeout(30);
  262 |     }
  263 |     await page.locator('button:has-text("VER MEU PERFIL")').click();
  264 | 
  265 |     // Wait for calculating spinner then results
```