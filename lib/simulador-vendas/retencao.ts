...
type Candidata = {
  ...
  hash: string;
  documento: {
    sessao: { empresa_id: string; id: string };
    tentativas: unknown[];
    revisoes: { empresa_id: string; sessao_id: string }[];
  };
};
...
export async function expurgarComBackup(
  ...
  if (
    candidata.documento.sessao.empresa_id !== empresaId ||
    candidata.documento.sessao.id !== candidata.id ||
    !Array.isArray(candidata.documento.revisoes) ||
    candidata.documento.revisoes.some(
      (r) => r.empresa_id !== empresaId || r.sessao_id !== candidata.id,
    )
  )
    throw new Error('PACE: snapshot fora do escopo');
  const { caminho } = await gravarBackupConferido(
    tdb.storage,
    'pace-retencao',
    { politica: '6 meses', ...candidata },
  );
  ...
  if (removido.error)
    throw new Error('PACE: falha ao expurgar o snapshot conferido');
  if (removido.data === false) {
    await removerBackupRecusado(tdb.storage, caminho);
    return false;
  }
  if (removido.data !== true)
    throw new Error(
      'PACE: resposta de expurgo desconhecida; backup preservado',
    );
  return true;
}

/** Núcleo headless; somente o cron autenticado o chama. Nunca retorna conversas. */
export async function executarRetencaoPace(
  ...
  let removidas = 0,
    ...
    empresas = 0;
  const falhas: string[] = [];
  const checkpoint = await registro
    .from('sim_vendas_manutencao')
    .select('cursor_empresa')
    .eq('chave', 'retencao')
    .maybeSingle();
  if (checkpoint.error)
    throw new Error('PACE: não foi possível recuperar o cursor da retenção');
  ...
  let parcial = false;
  const esgotado = () =>
    Date.now() >= deadline || removidas + alteradas >= 1000;
  ...
  const corte = new Date(Date.now() - BACKUP_DIAS * 86_400_000)
    .toISOString()
    .slice(0, 10);
  for (const prefixo of PREFIXOS_BACKUP_PACE) {
    for (;;) {
      ...
      const lista = await registro.storage.from('backups').list(prefixo, {
        limit: 100,
        sortBy: { column: 'name', order: 'asc' },
      });
      if (lista.error)
        throw new Error(
          'PACE: não foi possível conferir a retenção dos backups',
        );
      const vencidos = lista.data.filter(
        (f) =>
          /^\d{4}-\d{2}-\d{2}_[0-9a-f-]{36}\.json\.gz$/.test(f.name) &&
          f.name.slice(0, 10) < corte,
      );
      ...
      if (apagados.error)
        throw new Error('PACE: falha na retenção dos backups de segurança');
    }
  }
  varredura: for (;;) {
    if (esgotado()) {
    ...
    if (pagina.error)
      throw new Error('PACE: não foi possível enumerar empresas para retenção');
    ...
    for (const empresa of pagina.data || []) {
      ...
      try {
        for (;;) {
          ...
          const lote = await tdb.rpc('sim_vendas_retencao_lote', {
            p_empresa: empresa.id,
            p_limite: Math.min(5, 1000 - removidas - alteradas),
          });
          ...
          for (const candidata of lote.data as Candidata[]) {
            ...
            if (await expurgarComBackup(tdb, empresa.id, candidata))
              removidas++;
            else {
              alteradas++;
              houveMudanca = true;
    ...
    if ((pagina.data?.length || 0) < 200) {
      proximaEmpresa = null;
      break;
    }
    cursor = pagina.data!.at(-1)!.id;
  }
  const salvo = await registro.from('sim_vendas_manutencao').upsert({
    chave: 'retencao',
    cursor_empresa: proximaEmpresa,
    updated_at: new Date().toISOString(),
  });
  if (salvo.error)
    throw new Error('PACE: não foi possível salvar o cursor da retenção');
