...
**Simulador de liderança (documentação revisada em 20/09/2026):** jornada de cinco encontros conectados com a mesma equipe, nas rotas `/dashboard/simulador-lideranca` e `/admin/simulador-lideranca`, distinta do Mapeamento por cenários. Na jornada v2, cada encontro avalia uma competência em foco e duas secundárias (18 descritores); a média geral aparece só na síntese da jornada, e o encontro mostra o nível de cada competência (decisão do dono confirmada em 22/09/2026: a média pede 3 competências com nível, e o encontro avalia 3, então a caixa dela vivia vazia). Repetições preservam a sequência original; o acompanhamento usa o recorte das devolutivas, sem conversa integral. Não há revisão humana (decisão do dono, 22/09/2026); o que ficou dela no banco está no parágrafo seguinte. Regras e estado da entrega em [SIMULADOR-LIDERANCA.md](SIMULADOR-LIDERANCA.md); ensaios e dependência de banco em [simuladores-validacao.md](simuladores-validacao.md).

**Os três simuladores (vendas, atendimento, liderança), revisão de 18 e 19/09/2026:** núcleo comum em `lib/simuladores/` (regra de cobertura, citação tolerante, evolução só por avanço, relatório por competência e acesso por cargo). Quem TREINA sai da aba Simuladores de `/admin/cargos` (`sys_config.simuladores_por_cargo`, por id de cargo), e o cargo do cadastro da pessoa casa pelo nome normalizado (`chaveCargo`: caixa, acento e espaço; o nome exato vence), a mesma régua no gate e nos três painéis de equipe. Até 19/09 o gate comparava o nome exato e, sem achar o cargo, liberava os três simuladores. A mesma aba abre com o resumo "Simuladores da empresa": o que impede todo mundo de treinar (prazo do vendas, segmento do atendimento, programa da liderança), cargos liberados e treinos iniciados em 30 dias, sem os de administrador da plataforma; cargo-âncora da matriz de liderança fica fora da tabela. **A aba diz só quem TREINA (decisão do dono, 22/09/2026):** RH e gestor acompanham os resultados da equipe nos três simuladores sem depender dela, e o Mapeamento de liderança é do RH pelo papel e pelo módulo contratado. Até 22/09 a coluna Liderança do cargo do próprio RH decidia a tela, os relatórios, os PDFs e o menu do Mapeamento; desmarcá-la para testar quem treina tirava o Mapeamento do RH. **Sem revisão humana (mesma decisão):** o parecer de quem acompanha existiu nos três até 22/09 e saiu do app inteiro (rotas, núcleo `lib/simuladores/revisao*`, componentes e textos). As tabelas `recepcao_revisoes` (mig 241), `sim_vendas_revisoes` e `sim_lideranca_revisoes` (mig 264) ficaram no banco sem escritor, com 0 linhas medidas em 22/09, porque as funções de retenção e exclusão da mig 265 as leem; apagá-las exige reescrever essas funções numa migration nova. Detalhe por simulador: [SIMULADOR-VENDAS.md](SIMULADOR-VENDAS.md), [recepcao-medica.md](recepcao-medica.md) e [SIMULADOR-LIDERANCA.md](SIMULADOR-LIDERANCA.md).
...
├── migrations/                   # 249 arquivos SQL (000 + 022 -> 267, com gaps)
...
**Nome (19/09/2026).** Decisão do dono de 18/09: a tela passa a se chamar "Mapeamento de liderança" nas telas e na proposta; rotas, chaves `prontidao_*` e o módulo `prontidao_lideranca` ficam como estão. O "Simulador de liderança" é outra coisa, a jornada de cinco encontros ([SIMULADOR-LIDERANCA.md](SIMULADOR-LIDERANCA.md)), ligada pelo mesmo módulo. Na aba Simuladores de `/admin/cargos`, a coluna Liderança libera só o simulador para quem treina. **Acesso (22/09/2026):** este mapeamento é do RH pelo papel e pelo módulo contratado, sem olhar a aba de cargos: a página (`lib/simuladores/pagina.ts`), as quatro actions do RH com painel, relatórios e PDFs (`ctxRh` em `actions/prontidao-lideranca.ts`) e o item do menu (`app/api/me/route.ts`). Até essa data a coluna do cargo do próprio RH também decidia essas portas.
