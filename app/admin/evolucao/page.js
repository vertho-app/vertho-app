...
function ResumoGeral({ porCompetencia }) {
  const stats = { evolucao_confirmada: 0, evolucao_parcial: 0, estagnacao: 0, regressao: 0 };
...
function CompetenciaCard({ nome, descritores, expanded, onToggle }) {
  const descs = Object.entries(descritores);
  const totalAvaliacoes = descs.reduce((sum, [, d]) => sum + (d.evolucao_confirmada + d.evolucao_parcial + d.estagnacao + d.regressao), 0);
