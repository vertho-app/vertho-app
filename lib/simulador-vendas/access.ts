...
export function empresaAutorizada(auth: AuthenticatedContext, solicitada?: string | null) {
  ...
  if (!auth.isPlatformAdmin && ((solicitada && solicitada !== empresaId) || !auth.colaborador || auth.colaborador.empresa_id !== empresaId)) {
    throw new SimuladorError(403, 'Seu cadastro não tem acesso a esta empresa.');
  }
  return empresaId;
}
export async function contexto(req: Request, empresa?: string | null, escrita = false, autenticado?: AuthenticatedContext) {
  const auth = autenticado ?? await requireUser(req);
  if (auth instanceof Response) return auth;
  const empresaId = empresaAutorizada(auth, empresa);
  if (escrita && !(await can(auth, 'assessments.answer'))) throw new SimuladorError(403, 'Seu perfil não permite realizar treinos.');
  const tdb = tenantDb(empresaId);
  const [empresaResult, configResult] = await Promise.all([
    tdb.raw.from('empresas').select('id,nome').eq('id', empresaId).maybeSingle(),
    tdb.from('sim_vendas_config').select('habilitado,briefing,limite_sessoes').maybeSingle(),
  ]);
  if (empresaResult.error || configResult.error) throw new SimuladorError(503, 'Não foi possível consultar o simulador. Tente novamente.');
  if (!empresaResult.data) throw new SimuladorError(404, 'Empresa não encontrada.');
  const config = configResult.data as Config | null;
  if (!auth.isPlatformAdmin && !config?.habilitado) throw new SimuladorError(403, 'O simulador de vendas ainda não está habilitado para sua empresa.');
  let ownerKey: string;
  if (auth.isPlatformAdmin) {
    const { data, error } = await tdb.raw.from('platform_admins').select('id').eq('email', auth.email.toLowerCase()).maybeSingle();
  ...
  return { auth, empresaId, empresaNome: empresaResult.data.nome, config, ownerKey, tdb,
...
export async function vendasHabilitado(empresaId?: string | null): Promise<boolean> {
  ...
  try {
    const { data, error } = await tenantDb(empresaId).from('sim_vendas_config').select('habilitado').maybeSingle();
