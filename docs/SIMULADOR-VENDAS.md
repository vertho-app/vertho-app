...
- **Sem revisão humana** (decisão do dono, 22/09/2026): a gestão abre o relatório de cada treino e não registra parecer; `/api/simulador-vendas/gestao` só lê. A revisão existiu de 19 a 22/09 (mig 264); a tabela `sim_vendas_revisoes` ficou no banco sem escritor, porque a retenção e a exclusão da mig 265 a leem.
- **Quem acompanha não depende da aba de cargos** (mesma decisão): a aba Simuladores de `/admin/cargos` diz só quem TREINA; gestor e RH veem a equipe pelo papel.
