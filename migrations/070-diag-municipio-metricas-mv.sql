-- ═════════════════════════════════════════════════════════════════
-- Migration 070 — MV consolidada de métricas por município
-- Tabela base para a tabela comparativa Cidade × Microrregião × UF × BR.
-- Cada linha = 1 município com seus indicadores principais já agregados.
-- Médias por microrregião/UF/Brasil são calculadas on-the-fly em cima
-- desta MV (são poucos milhares de linhas, custo desprezível).
-- ═════════════════════════════════════════════════════════════════
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_diag_mv_mun_metricas_pk
  ON diag_mv_municipio_metricas(municipio_ibge);
CREATE INDEX IF NOT EXISTS idx_diag_mv_mun_metricas_uf
  ON diag_mv_municipio_metricas(uf);
CREATE INDEX IF NOT EXISTS idx_diag_mv_mun_metricas_micro
...
-- ─────────────────────────────────────────────────────────────────
-- Função RPC para retornar a tabela comparativa de um município
-- 4 linhas: cidade, microrregião, UF, BR (médias agregadas)
-- ─────────────────────────────────────────────────────────────────

CREATE OR REPLACE FUNCTION diag_municipio_benchmarks(p_ibge TEXT)
RETURNS TABLE (
  scope        TEXT,
  ica_taxa     NUMERIC,
  ideb_5ef     NUMERIC,
...
-- Atualiza o refresh helper para incluir a nova MV.
