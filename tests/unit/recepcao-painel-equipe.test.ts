...
const CODIGOS = ['acolhimento', 'compreensao', 'clareza', 'resolucao', 'procedimentos'];
const sessao = (colaborador: string, data: string, notas: Record<string, number | null> | null, status = 'concluida') => ({
  colaborador_id: colaborador,
  created_at: data,
  estado: {
    status,
    relatorio: notas
      ? { escalaNota: '1-4', competencias: Object.entries(notas).map(([codigo, nota]) => ({ codigo, nota })) }
      : null,
  },
});
...
describe('visaoPorCompetencia', () => {
  ...
  it('maior nível por competência, só avanço; sem treino no período fica listado', () => {
    ...
    expect(ana).toMatchObject({ iniciadas: 2, concluidas: 2, ultimo: '2026-09-15' });
    expect(ana.competencias.find((c) => c.codigo === 'acolhimento')).toMatchObject({ nivelAlcancado: 3, subiu: true });
    expect(ana.competencias.find((c) => c.codigo === 'compreensao')).toMatchObject({ nivelAlcancado: 3, subiu: false });
    expect(v.naoTreinaram.map((p) => p.id)).toEqual(['caio']);
    expect(v.competencias.find((c) => c.codigo === 'acolhimento')!.niveis).toEqual([0, 0, 1, 0]);
  });

  it('relatório sem matriz (escala antiga) conta como treino, mas não dá nível', () => {
    const antigo = sessao('bia', '2026-09-12', null);
    antigo.estado.relatorio = { competencias: [{ codigo: 'acolhimento', nota: 3 }] } as any;
...
describe('populacaoAtendimento', () => {
  const colaboradores = [
    { id: 'ana', nome_completo: 'Ana', cargo: 'Recepção', email: 'ana@c.test', role: 'colaborador', gestor_email: 'g@c.test' },
    { id: 'bia', nome_completo: 'Bia', cargo: 'Recepção', email: 'bia@c.test', role: 'colaborador', gestor_email: 'outro@c.test' },
    { id: 'caio', nome_completo: 'Caio', cargo: 'Financeiro', email: 'caio@c.test', role: 'colaborador', gestor_email: 'g@c.test' },
    { id: 'gil', nome_completo: 'Gil', cargo: 'Recepção', email: 'g@c.test', role: 'gestor', gestor_email: null },
    { id: 'sup', nome_completo: 'Suporte', cargo: 'Recepção', email: 'sup@vertho.ai', role: 'colaborador', gestor_email: 'g@c.test' },
  ...
  const ctx = (role: string, admin = false) =>
    ({
      empresaId: EMPRESA,
      sb: sb.client,
      auth: { role, isPlatformAdmin: admin, empresaId: EMPRESA, email: 'g@c.test', colaborador: { id: 'gil', empresa_id: EMPRESA, email: 'g@c.test' } },
    }) as any;

  beforeEach(() => {
    sb = criarSupabaseMock({
      resolver: (t) =>
        t === 'empresas' ? { sys_config: { simuladores_por_cargo: { 'c-rec': { atendimento: true }, 'c-fin': { atendimento: false } } } } : null,
      lista: (t, _c, cadeia) =>
        t === 'cargos_empresa'
          ? [{ id: 'c-rec', nome: 'Recepção' }, { id: 'c-fin', nome: 'Financeiro' }]
          : t === 'colaboradores'
            ? colaboradores.filter((p) => p.empresa_id === valor(cadeia, 'empresa_id'))
            : [],
    });
  });

  it('gestor: liderados com o cargo liberado; gestor e contas internas não entram', async () => {
    expect((await populacaoAtendimento(ctx('gestor'))).map((p) => p.id)).toEqual(['ana']);
  });
