...
export function EditorCenario({registro,biblioteca=[],busy,salvar,fechar}:{registro:any;biblioteca?:Biblioteca;busy:boolean;salvar:(cmd:any)=>Promise<any>;fechar:()=>void}) {
 ...
 return <div className={styles.editor}>
  ...
  <fieldset disabled={busy||registro.estado==='publicado'}>
   <div className={styles.formGrid}><label>Identificador do caso<input value={c.id} onChange={e=>editar(n=>{n.id=e.target.value})} placeholder="exemplo-remarcacao"/></label><label>Título<input value={c.publico.titulo} onChange={e=>editar(n=>{n.publico.titulo=e.target.value})}/></label><label>Clínica fictícia<input value={c.publico.clinica||''} onChange={e=>editar(n=>{n.publico.clinica=e.target.value})}/></label><label>Canal<select value={c.publico.canal} onChange={e=>editar(n=>{n.publico.canal=e.target.value as 'mensagens'|'telefone'})}><option value="mensagens">Mensagens / WhatsApp simulado</option><option value="telefone">Telefone simulado</option></select></label><label>Nível<select value={c.publico.nivel||''} onChange={e=>editar(n=>{n.publico.nivel=(e.target.value||undefined) as Cenario['publico']['nivel']})}><option value="">Sem nível (fim da lista)</option>{NIVEIS.map(n=><option key={n} value={n}>{rotuloNivel[n]}</option>)}</select></label><label>Limite de respostas<input type="number" min="1" max="20" value={c.limiteRespostas} onChange={e=>editar(n=>{n.limiteRespostas=Number(e.target.value)})}/></label><label>Referência de data e horário<input value={c.publico.agora||''} onChange={e=>editar(n=>{n.publico.agora=e.target.value||undefined})}/></label></div>
   ...
   <p className={styles.small}>Marque as competências da biblioteca que entram na análise deste caso e distribua os pesos. O comportamento esperado em cada nível vem da biblioteca e é copiado para o caso ao salvar; o critério diz o que observar neste caso. De 3 a 7 competências.</p>
   ...
   {linhasRubrica.map(l=><div key={l.id} className={styles.group}>
    ...
    {l.marcada&&<label>Critério neste caso (o que observar)<textarea value={l.dim!.criterio} onChange={e=>editarDim(l.id,'criterio',e.target.value)}/></label>}
