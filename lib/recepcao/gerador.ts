...
export function codigoFalha(e: unknown) {
  if (e instanceof ErroReferenciaAvaliacao)
    return { codigo: e.codigo, campo: e.campo };
  if (e instanceof z.ZodError)
    return {
      codigo: 'schema_invalido',
      campo: e.issues[0]?.path
        .map((p) =>
          typeof p === 'number' ? p : String(p).replace(/[^a-zA-Z_]/g, ''),
        )
        .join('.'),
    };
  if (e instanceof SyntaxError) return { codigo: 'json_invalido', campo: null };
  if (e instanceof Error && e.message === 'Exposição de instruções na fala')
    return { codigo: 'instrucoes_expostas', campo: null };
  return { codigo: 'geracao_ou_regra', campo: null };
}
export function geradorRecepcao(
  empresaId: string | null,
  colaboradorId: string | null,
  admin = false,
  persistencia?: {
    sb: ContextoRecepcao['sb'];
    sessaoId: string;
  ...
  async function validar(erro?: unknown) {
    ...
    if (persistencia) {
      const { error } = await persistencia.sb
        .from('recepcao_tentativas')
        .update({
          estado: falha ? 'rejeitada' : 'aceita',
          erro_codigo: falha?.codigo || null,
          erro_campo: falha?.campo || null,
          finished_at: new Date().toISOString(),
          duracao_ms: Date.now() - atual.inicio,
        })
        .eq('empresa_id', empresaId)
        .eq('sessao_id', persistencia.sessaoId)
        .eq('id', atual.id);
      if (error)
        console.error('[recepcao] não foi possível finalizar telemetria', {
          tentativa: atual.id,
        });
    }
    atual.finalizada = true;
  }
  const gerarTentativa: Gerar = async ({
    etapa,
    system,
    messages,
    perfilPaciente,
    ...
    if (persistencia) {
      const { error } = await persistencia.sb
        ...
        .insert({
          ...
          sessao_id: persistencia.sessaoId,
          etapa,
          modelo_solicitado: model,
          prompt_hash: chamada.promptHash,
          prompt_versao: chamada.promptVersion,
          cenario_versao: persistencia.cenarioVersao,
        });
      if (error) {
        atual.finalizada = true;
        throw new Error('Não foi possível registrar a tentativa de IA.');
      }
    }
    ...
    try {
      const raw = await callAIChat(
        ...
        messages,
        { model },
        etapa === 'paciente' ? 4000 : 16000,
        {
          taskKey,
          empresaId,
          colaboradorId,
          source: admin ? 'piloto' : 'wrapper',
          locale: 'pt-BR',
      ...
      return JSON.stringify(
        (etapa === 'paciente' ? pacienteSchema : avaliacaoSchema).parse(
          parseJsonIA(raw),
        ),
      );
    } catch (e) {
  ...
  const gerar: Gerar = async (args) => {
    try {
      return await gerarTentativa(args);
    } catch (e) {
