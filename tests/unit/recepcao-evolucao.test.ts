...
test('indicadores não misturam casos nem versões, e a cobertura não fragmenta o caso (18/09)',async()=>{
 ...
 const resumo=resumirEquipe(rows,[{id:'c',nome_completo:'Pessoa',ativo:true}]);
 expect(resumo.grupos).toHaveLength(2);expect(resumo.grupos.find(g=>g.chave.startsWith(s.cenario.id+'|')).sessoes).toBe(2);
});
