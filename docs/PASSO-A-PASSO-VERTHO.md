...
- A cédula é a **Top 10 do próprio cargo** (desde 25/09/2026; antes era a matriz inteira): cada colab escolhe 5 e ordena (1ª = 5 pontos … 5ª = 1), gerando ranking de relevância percebida. Regra em `lib/votacao/cedula.ts`
- A cédula aparece em **ordem alfabética**, não na ordem da IA, para não puxar o voto
...
4. **Colaborador**: diagnóstico completo inalterado (DISC/mapeamento/DNA/Fit) → sems 1-2 com 2 conteúdos + reflexão socrática cada (a IA cobra os DOIS desafios) → ao concluir a sem 2, a reflection marca a linha da acumulada como `processing` e **dispara a task `trigger/acumulada-piloto.ts` (Trigger.dev)** que roda a acumulada single-comp em background (migration 169). *Antes rodava num `after()` da reflection — frágil (morria no freeze da Vercel, corria com o fechamento → bugs B2/R1). Agora é task com retry.* **⚠️ Deploy da task é MANUAL** (`npx trigger.dev deploy`, receita com caminho sem espaço — o git push só deploya a Vercel)
...
- **26-27/07 — uma fonte de contexto institucional por empresa** (`062dca13` + 27/07): empresa-rede tem 1 PPP por escola (Ibipeba: 11), e o `.limit(1)` autorava a régua do município inteiro com uma escola sorteada. Classe **F-I10** no `docs/FMEA-PIPELINE.md`, agora fechada em **9 sites** (valores do IA2, `buscarContextoPPP` de IA1/IA2/IA3, check dual do IA3, PDF personalizado, IA4 ×2, Cenário B do fechamento ×3) e com **guard de CI** — régua, cenário, kit e PDF na mesma lente. No 26/07, os 2 guards de tenant voltaram ao verde (`3367efb7`).
- **27/07 (noite) — faxina do FMEA: zero 🔴/🟠 abertos** (`75764ed4` + `fc25fe36`): F-C6 (dedup + UNIQUE em `micro_conteudos` não-kit, mig 190 — com 17 planos reapontados antes do delete) e os 6 laranjas de código (F-C1 upsert do header, F-I2 reparo pelo motor, F-I4 filtro `disc` sem DDL, F-I5 FKs mig 191, F-I6 descritor canônico + backfill de 122 linhas, F-P1 denominador fixo + `parcial`, F-E4 lote síncrono vira stub). **F-I8 virou decisão de design**: conteúdo por IA usa a 1ª letra do DISC (célula de custo), relatório usa o combo. Detalhe, guardas e follow-ups no `docs/FMEA-PIPELINE.md`.
- **27/07 (noite, 2ª leva)** (`02b42068` + `ccba5c15`): **IA4 com retry self-service** — resposta avaliada sem notas volta para a fila (0 presas em produção); **overlay determinístico** (ORDER BY nos 2 resolvedores de kit); **§6 do FMEA** sincronizada e as **7 divergências doc×código** corrigidas (PIPELINE-TRILHA/KIT-SEMANAL: 3 formatos não 4, 12+4 não 16). Armadilha catalogada: `try/catch` não pega erro de query do supabase-js (ele retorna `{ error }`) — regra no CLAUDE.md.
...
- **Portal do Representante — documento da proposta** (`33c0e713`, `3316392f`, `66bf8e6e`, `d6fe08a9`, `f2e08540`, `8bbcbf1b`, `2de61dd2`): página pública `/proposta/[token]` + PDF **redesenhados** (visual claro/editorial, mais profissional pro cliente); **simulador de preço** (`lib/sales/pricing.ts`: valor mensal automático das variáveis + valor final após desconto, com o valor mensal em destaque); **versionamento de proposta** (migration 168) — de uma proposta enviada/aprovada o RC cria "Nova versão" (cópia editável, número `-Rn`) que segue de novo pela aprovação e a original vira **"Substituída"** (`superseded`); correção dos toasts do portal do RC (antes "submeter para aprovação" não dava feedback).
- **ACME Demo** (`27c9ded8`, `fcbf499a`, `17b19ce8`, `f97dd8e5`, `fcf50bbc`, `1fc29497`): 6 personas no portal `/representante/demo` (Mariana/Financeiro e Renato/Operações adicionadas), Gerente Comercial com pacote completo (5 comps + descritores + gabaritos IA2 + cenários IA3), e ranking de Adequação real e diferenciado (Mariana 92 "Alta"; Renato 88 e Carla 88 "Excelente"; Bruna 50 "Baixa"). Reset da demo: `npm run reset:demo` (ou botão em `/admin/demo`). **⚠️ O fit NÃO é pré-computado no reset** — para o ranking aparecer, clicar **"Calcular Fit"** 1× por cargo em `/admin/fit`.
