...
import {
  expurgarComBackup,
  executarRetencaoPace,
} from '@/lib/simulador-vendas/retencao';
let sb: SupabaseMock;
...
describe('retenção PACE: expurgo só depois de backup privado conferido', () => {
  ...
  it('confere bytes antes de expurgar o ID/hash/tenant exatos', async () => {
    expect(
      await expurgarComBackup(tenantDb(empresa), empresa, candidata()),
    ).toBe(true);
    ...
    expect(download.mock.invocationCallOrder[0]).toBeLessThan(
      sb.client.rpc.mock.invocationCallOrder[0],
    );
  ...
  it('falha de upload não autoriza DELETE', async () => {
    ...
    await expect(
      expurgarComBackup(tenantDb(empresa), empresa, candidata()),
  ...
  it('snapshot de outra empresa falha antes do backup', async () => {
    ...
    await expect(
      expurgarComBackup(tenantDb(empresa), empresa, c),
  ...
  it('timeout de expurgo é ambíguo e mantém o backup', async () => {
    sb.client.rpc.mockResolvedValueOnce({
      data: null,
      error: { message: 'timeout' },
    });
    await expect(
      expurgarComBackup(tenantDb(empresa), empresa, candidata()),
    ).rejects.toThrow('falha ao expurgar');
  ...
  it('nenhuma sessão vencida: zero escrita/descarte', async () => {
    ...
    expect(await executarRetencaoPace(sb.client)).toMatchObject({
      removidas: 0,
      empresas: 1,
    });
    expect(upload).not.toHaveBeenCalled();
  ...
  it('orçamento global para em 1.000 sem marcar outras empresas como erro', async () => {
    ...
    sb = criarSupabaseMock({
      lista: (t) =>
        t === 'empresas'
          ? [{ id: empresa }, { id: '10000000-0000-4000-8000-000000000003' }]
          : [],
    });
    sb.client.storage = storage;
    sb.client.rpc.mockImplementation(async (nome) => ({
      data:
        nome === 'sim_vendas_retencao_lote'
          ? Array.from({ length: 5 }, candidata)
          : true,
      error: null,
    }));
    ...
    expect(
      sb.client.rpc.mock.calls.every((c) => c[1].p_empresa === empresa),
    ).toBe(true);
    expect(logAdminAction).toHaveBeenLastCalledWith(
      expect.objectContaining({
        resultado: 'parcial',
        detalhes: expect.objectContaining({ falhas: 0 }),
      }),
    );
    expect(sb.escritas).toContainEqual(
      expect.objectContaining({
        tabela: 'sim_vendas_manutencao',
        payload: expect.objectContaining({ cursor_empresa: null }),
      }),
    );
  });
  ...
  it('checkpoint faz a rodada seguinte começar depois da empresa anterior', async () => {
    const storage = sb.client.storage;
    sb = criarSupabaseMock({
      resolver: () => ({ cursor_empresa: empresa }),
      lista: () => [{ id: '10000000-0000-4000-8000-000000000003' }],
    });
    sb.client.storage = storage;
    sb.client.rpc.mockResolvedValue({ data: [], error: null });
    expect(await executarRetencaoPace(sb.client)).toMatchObject({
  ...
  it('falha de checkpoint impede fingir uma manutenção completa', async () => {
    ...
    await expect(executarRetencaoPace(sb.client)).rejects.toThrow(
      'recuperar o cursor',
    );
    expect(upload).not.toHaveBeenCalled();
  });
