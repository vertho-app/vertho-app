-- ═════════════════════════════════════════════════════════════════
-- Migration 071 — MV consolidada de métricas por escola (INEP)
-- Análoga à 070, mas no nível da escola. Permite comparar uma escola
-- com a média das demais escolas da mesma microrregião e do estado.
-- ═════════════════════════════════════════════════════════════════
...
-- ─────────────────────────────────────────────────────────────────
-- RPC: 3 linhas — escola, microrregião (média de outras escolas),
-- estado (média de outras escolas).
-- ─────────────────────────────────────────────────────────────────

CREATE OR REPLACE FUNCTION diag_escola_benchmarks(p_inep TEXT)
RETURNS TABLE (
  scope         TEXT,
  ideb_5ef      NUMERIC,
  ideb_9ef      NUMERIC,
