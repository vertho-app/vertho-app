...
import { DISCOVERY_CHECKLIST, PACE_PHASES, type CopilotPlan, type DiscoveryKey, type PacePhase } from '@/lib/copiloto/types';

export const runtime = 'nodejs';
export const dynamic = 'force-dynamic';
export const maxDuration = 300;

const MAX = { company: 200, site: 320, context: 30000, offer: 12000 } as const;
...
function normalizePlan(research: any, synthesis: any, sources: Array<{ title: string; url: string }>): CopilotPlan {
  ...
  for (const source of sources) {
    ...
    if (url) sourceMap.set(url, { title: text(source.title, 240) || url, url });
  }

  const facts = (Array.isArray(research?.fatos_relevantes) ? research.fatos_relevantes : []).slice(0, 8).map((item: any) => {
  ...
  return {
    ...
    facts,
    trends: (Array.isArray(research?.tendencias_setor) ? research.tendencias_setor : []).slice(0, 6).map((item: any) => ({
      title: text(item?.titulo, 240), impact: text(item?.impacto, 900), sourceUrl: safeUrl(item?.fonte_url),
...
export async function POST(req: Request) {
  try {
    ...
    if (company.length < 2 && site.length < 4 && context.length < 20 && !opportunityId) {
      return NextResponse.json({ error: 'Informe a empresa, o site ou um briefing com pelo menos 20 caracteres' }, { status: 400 });
    }
    ...
    if (company.length >= 2 || site.length >= 4) {
      const result = await researchCompany(company, site);
    ...
    return NextResponse.json({ plan: normalizePlan(research, synthesis, sources) });
  } catch (error: any) {
