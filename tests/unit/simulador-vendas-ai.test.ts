...
describe('checkpoint dos agentes PACE', () => {
  beforeEach(() => { sb = criarSupabaseMock(); vi.mocked(callAI).mockReset().mockResolvedValue(JSON.stringify(semViolacao)); });
  ...
  it('reutiliza saída aceita e aplica de novo a validação sem gastar IA', async () => {
    sb = criarSupabaseMock({ lista: () => [{ tentativa: 1, prompt_hash: hashPrompt('PROMPT_PRIVADO'), modelo: 'gpt-5.4-2026-03-05', status: 'aceita', resultado: semViolacao }] });
    const validar = vi.fn();
    expect(await gerador(ctx(), estado(), req)('moderador', {}, validar)).toEqual(semViolacao);
    expect(validar).toHaveBeenCalledOnce(); expect(callAI).not.toHaveBeenCalled();
    expect(sb.chamadas).toContainEqual({ tabela: 'sim_vendas_tentativas', metodo: 'eq', args: ['empresa_id', 'empresa-a'] });
  });
  it('mesmo request com prompt diferente é conflito antes do provedor', async () => {
    sb = criarSupabaseMock({ lista: () => [{ tentativa: 1, prompt_hash: 'outro', modelo: 'gpt-5.4-2026-03-05', status: 'aceita', resultado: semViolacao }] });
  ...
  it('chamada aceita carrega tenant, correlação e snapshot; checkpoint precede retorno', async () => {
    vi.mocked(callAI).mockImplementation(async () => {
      expect(sb.escritas).toHaveLength(1); expect(sb.escritas[0].payload).toMatchObject({ empresa_id: 'empresa-a', request_id: req });
      return JSON.stringify(semViolacao);
    });
    await gerador(ctx(), estado(), req)('moderador', {});
    expect(callAI).toHaveBeenCalledWith('', 'PROMPT_PRIVADO', { model: 'gpt-5.4-2026-03-05' }, 2500, expect.objectContaining({ empresaId: 'empresa-a', colaboradorId: 'colab-a', correlationId: sb.escritas[0].payload.id, maxRetries: 0 }));
  ...
  it('falha ao salvar nunca retorna sucesso nem refaz chamada paga automaticamente', async () => {
    sb.falharEm({ tabela: 'sim_vendas_tentativas', op: 'update', quando: p => p.status === 'aceita', mensagem: 'timeout' });
    await expect(gerador(ctx(), estado(), req)('moderador', {})).rejects.toMatchObject({ status: 503 });
